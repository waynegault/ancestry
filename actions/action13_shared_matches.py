#!/usr/bin/env python3
"""
Action 13: Shared Match Scraper
...
def fetch_shared_matches(session_manager: SessionManager) -> bool:
    """
    Main entry point for Action 13.
    """
    log_action_banner("ACTION 13: SHARED MATCH SCRAPER", "start")

    # Ensure session is ready (browser + API)
    if not session_manager.ensure_session_ready():
        logger.error("Session not ready. Aborting.")
        log_action_banner("ACTION 13: SHARED MATCH SCRAPER", "failure")
        return False

    try:
        _process_shared_matches(session_manager)
        log_action_banner("ACTION 13: SHARED MATCH SCRAPER", "success")
        return True
    except Exception as e:
        logger.exception(f"Action 13 failed: {e}")
        log_action_banner("ACTION 13: SHARED MATCH SCRAPER", "failure")
        return False
...
def _store_shared_matches(
    ...
    with db_transn(db_session) as session:
        ...
        primary_person_id_stmt = select(DnaMatch.person_id).where(DnaMatch.id == primary_match_id)
...
def action13_module_tests() -> bool:
    from unittest.mock import MagicMock

    from testing.test_framework import TestSuite

    suite = TestSuite("Action 13 Shared Matches", __name__)
    suite.start_suite()
...
run_comprehensive_tests = create_standard_test_runner(action13_module_tests)
