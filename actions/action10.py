...
from performance.performance_cache import (
    fast_test_cache,
)
...
from genealogy.genealogy_presenter import display_family_members, present_post_selection
...
from testing.test_framework import mock_logger_context
...
def _perform_api_search_fallback(
    ...
    api_module = importlib.import_module("api.api_search_core")
    search_api = cast(
        SearchAPIFunc,
...
def _display_search_results(gedcom_matches: MatchList, api_matches: MatchList, max_to_show: int) -> None:
    ...
    if not left_rows and not right_rows:
        logger.info("Action 10: No matches found in GEDCOM or API.")
        print("")
        print("No matches found.")
        return

    logger.info(
        "Action 10: Found %d GEDCOM matches and %d API matches.",
        len(gedcom_matches),
        len(api_matches),
    )
...
def _execute_comparison_search(
    ...
    *,
    ...
    if gedcom_matches:
        ...
        top_match = gedcom_matches[0]
        score = top_match.get("total_score", 0.0)
        # Threshold: 50.0 (Moderate Confidence)
        if score >= 50.0:
            should_search_api = False
            logger.info("GEDCOM match found with score %.1f (>= 50.0). Skipping API search.", score)
        else:
            logger.info("GEDCOM match found but score %.1f is low (< 50.0). Performing API search.", score)
...
def _search_for_person(gedcom_data: Any, search_criteria: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    from testing.test_framework import clean_test_output
...
def test_fraser_gault_scoring_algorithm() -> None:
    ...
    from testing.test_framework import Colors, format_score_breakdown_table, format_search_criteria
...
def test_real_search_performance_and_accuracy() -> None:
    ...
    from testing.test_framework import Colors, clean_test_output, format_test_section_header
...
def _test_perform_api_search_fallback_returns_fallback_matches() -> None:
    class FakeSessionManager:
        def __init__(self) -> None:
            ...
            # Mock browser_manager.driver for _api_req
            self.browser_manager = SimpleNamespace(driver=SimpleNamespace(current_url="http://mock"))
    ...
    fake_api_module = ModuleType("api.api_search_core")
    setattr(fake_api_module, "search_ancestry_api_for_person", fake_search)

    with (
        _temporary_module("api.api_search_core", fake_api_module),
        _patched_globals(
            {
...
def _test_execute_comparison_search_skips_api_when_good_match_exists() -> None:
    config = ComparisonConfig(
        gedcom_path=None,
    ...
    def fake_gedcom(*_args: Any, **_kwargs: Any) -> tuple[Any, MatchList]:
        return ({"data": True}, [{"id": "G1", "total_score": 100.0}])
    ...
    assert results.gedcom_matches == [{"id": "G1", "total_score": 100.0}]
    assert results.api_matches == []
    assert not fallback_calls


def _test_execute_comparison_search_runs_api_when_poor_match_exists() -> None:
    config = ComparisonConfig(
        gedcom_path=None,
        reference_person_id_raw=None,
        reference_person_name=None,
    ...
    def fake_gedcom(*_args: Any, **_kwargs: Any) -> tuple[Any, MatchList]:
        return ({"data": True}, [{"id": "G1", "total_score": 20.0}])
    ...
    assert results.gedcom_matches == [{"id": "G1", "total_score": 20.0}]
    assert results.api_matches == [{"id": "API1"}]
...
def action10_module_tests() -> bool:
    ...
    suite.run_test(
        "Comparison execution (Good GEDCOM match)",
        _test_execute_comparison_search_skips_api_when_good_match_exists,
        "Ensures API search is skipped when a high-quality GEDCOM match exists.",
    )
    suite.run_test(
        "Comparison execution (Poor GEDCOM match)",
        _test_execute_comparison_search_runs_api_when_poor_match_exists,
        "Ensures API search runs when GEDCOM match quality is low.",
    )
    suite.run_test(
...
if __name__ == "__main__":
    ...
    if os.getenv("RUN_MODULE_TESTS", "0") == "1":
        ...
        from core.session_utils import get_session_manager, register_session_manager
