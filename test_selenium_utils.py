...
class TestSeleniumUtils(unittest.TestCase):
    ...
    def test_extract_text(self):
        ...
        self.mock_element.find_element.assert_called_once_with(By.CSS_SELECTOR, "div.test")
        ...
        self.mock_element.reset_mock()
        self.mock_element.find_element.side_effect = NoSuchElementException("Test exception")
    ...
    def test_extract_attribute(self):
        ...
        result = selenium_utils.extract_attribute(self.mock_element, "div.test", "data-test")
        ...
        child_element.reset_mock()
        self.mock_element.find_element.side_effect = NoSuchElementException("Test exception")
        result = selenium_utils.extract_attribute(self.mock_element, "div.nonexistent", "data-test")
        self.assertEqual(result, "")
        
        # Test with general exception
        self.mock_element.reset_mock()
        self.mock_element.find_element.side_effect = Exception("Test exception")
        result = selenium_utils.extract_attribute(self.mock_element, "div.error", "data-test")
        self.assertEqual(result, "")

    @patch('selenium.webdriver.support.wait.WebDriverWait')
    def test_is_elem_there(self, mock_wait_class):
        """Test is_elem_there function."""
        # Test successful element presence
        mock_wait = MagicMock()
        mock_wait_class.return_value = mock_wait
        
        result = selenium_utils.is_elem_there(self.mock_driver, By.ID, "test-id")
        ...
        mock_wait_class.assert_not_called()
        
        # Test with TimeoutException
        mock_wait_class.reset_mock()
        mock_wait = MagicMock()
        mock_wait_class.return_value = mock_wait
        mock_wait.until.side_effect = TimeoutException("Test timeout")
        
        result = selenium_utils.is_elem_there(self.mock_driver, By.ID, "nonexistent-id")
        self.assertFalse(result)
        ...
        mock_wait = MagicMock()
        mock_wait_class.return_value = mock_wait
        mock_wait.until.side_effect = Exception("Test exception")
        
        result = selenium_utils.is_elem_there(self.mock_driver, By.ID, "error-id")
        self.assertFalse(result)
