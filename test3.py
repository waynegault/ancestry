...
try:
    ...
    logger.error(
        "Could not import SessionManager from utils. Make sure the utils module is available."
    )
    sys.exit(1)
...
def test_api_endpoints():
    ...
    if owner_tree_id and owner_profile_id:
        ...
        logger.warning(
            "Cannot construct owner facts referer: Tree ID or Profile ID missing."
        )
        owner_facts_referer = base_url  # Fallback

    # Test different API endpoints and parameter formats - focusing on treesui-list endpoint
    test_cases = [
        # Test 1: Try the treesui-list endpoint from the original URL
        {
            "description": "treesui-list endpoint",
            "url": f"{base_url}/api/treesui-list/trees/{tree_id}/persons",
            "params": {"fn": "", "ln": "gault"},
        },
        # Test 2: Try treesui-list with birth year parameter
        {
            "description": "treesui-list endpoint with birth year parameter",
            "url": f"{base_url}/api/treesui-list/trees/{tree_id}/persons",
            "params": {"fn": "", "ln": "gault", "by": "1941"},
        },
        # Test 3: Try treesui-list with different birth year parameter format
        {
            "description": "treesui-list endpoint with different birth year parameter format",
            "url": f"{base_url}/api/treesui-list/trees/{tree_id}/persons",
            "params": {"fn": "", "ln": "gault", "birthyear": "1941"},
        },
        # Test 4: Try treesui-list with birth year range
        {
            "description": "treesui-list endpoint with birth year range",
            "url": f"{base_url}/api/treesui-list/trees/{tree_id}/persons",
            "params": {"fn": "", "ln": "gault", "bymin": "1940", "bymax": "1942"},
        },
        # Test 5: Try treesui-list with different birth year range format
        {
            "description": "treesui-list endpoint with different birth year range format",
            "url": f"{base_url}/api/treesui-list/trees/{tree_id}/persons",
            "params": {
                "fn": "",
                "ln": "gault",
            ...
            if isinstance(data, list):
                ...
                for j, person in enumerate(data):
                    ...
                    if "fraser" in name.lower() and "gault" in name.lower():
                        fraser_found = True
                        logger.info(
                            f"Found Fraser Gault at position {j+1} with birth year {birth_year}"
                        )
                        print(
                            f"Found Fraser Gault at position {j+1} with birth year {birth_year}"
                        )
