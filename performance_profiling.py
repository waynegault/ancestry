...
def configure_profiling(
    ...
    logger.info(
        f"Profiling configured: enabled={enabled}, output_dir={_profile_config.output_dir}"
    )
...
def profile_with_cprofile(
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            profiling_active = (
                enabled if enabled is not None else _profile_config.enabled
            )
            ...
            logger.info(
                f"📊 Profiling {func.__name__} → {stats_path}"
            )
            ...
            try:
                ...
                if _profile_config.generate_txt_report:
                    _generate_txt_report(
                        stats_path, sort_by or _profile_config.sort_by
                    )
                ...
                profiler.dump_stats(str(stats_path))
                logger.warning(
                    f"⚠️  Function failed but profile saved: {stats_path}"
                )
                raise e
...
def time_function(func: Callable[..., R]) -> Callable[..., R]:
    ...
    def wrapper(*args: Any, **kwargs: Any) -> R:
        ...
        try:
            ...
            duration = time.perf_counter() - start_time
            logger.error(
                f"⏱️  {func.__name__} failed after {duration:.2f}s: {e}"
            )
            raise
...
def module_tests() -> bool:
    ...
    suite.run_test(
        ...
        test_time_decorator,
        "Test lightweight timing decorator"
    )
    ...
    suite.run_test(
        "Profile Decorator (Disabled)",
        test_profile_disabled,
        "Test cProfile decorator when profiling is disabled"
    )
    ...
    def test_profile_enabled():
        ...
        expected = 42 + 499500
        assert result == expected, f"profile_with_cprofile (enabled) should preserve return value (got {result}, expected {expected})"
    ...
    suite.run_test(
        "Profile Decorator (Enabled)",
        test_profile_enabled,
        "Test cProfile decorator when profiling is enabled"
    )
    ...
    suite.run_test(
        "Profile Output Path",
        test_output_path,
        "Test get_profile_output_path function"
    )
    ...
    suite.run_test(
        ...
        "Test generate_report_from_stats function"
    )
