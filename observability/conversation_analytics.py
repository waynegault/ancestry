...
def _update_research_outcomes(
    metrics: ConversationMetrics, person_looked_up: bool, person_found: bool, research_task_created: bool
) -> None:
...
def get_overall_analytics(session: Session) -> dict[str, Any]:
    ...
    avg_time_to_response = (
        session.query(func.avg(ConversationMetrics.time_to_first_response_hours))
        .filter(ConversationMetrics.time_to_first_response_hours.isnot(None))
        .scalar()
        or 0
    )
...
def _test_update_conversation_metrics_existing() -> None:
    ...
    try:
        ...
        metrics2 = update_conversation_metrics(
            session=session, people_id=person.id, message_received=True, engagement_score=60
        )
...
def _test_template_tracking() -> None:
    ...
    try:
        ...
        update_conversation_metrics(session=session, people_id=person.id, message_sent=True, template_used="template1")

        metrics = update_conversation_metrics(
            session=session, people_id=person.id, message_sent=True, template_used="template2"
        )
...
def _test_research_outcomes_tracking() -> None:
    ...
    try:
        ...
        metrics = update_conversation_metrics(
            session=session, people_id=person.id, person_looked_up=True, person_found=True, research_task_created=True
        )
