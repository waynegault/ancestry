...
# --- GEDCOM library availability and imports (moved from gedcom_interrogator.py) ---
import logging
...
def is_elem_there(
    driver: Optional[WebDriver], by: str, value: str, wait: int = 2
) -> bool:
    """
...
class SessionManager:
    ...
    def _check_and_handle_url(self) -> bool:
        ...
        try:
            ...
            AttributeError
        ):  # Handle case where self.driver might become None unexpectedly
    ...
    def get_cookies(self, cookie_names: List[str], timeout: int = 30) -> bool:
        ...
        try:
            ...
            if self.driver and self.is_sess_valid():  # Use is_sess_valid for robustness
                ...
                missing_final = (
                    cookie_names  # Assume all missing if driver gone/invalid
                )
...
def _fetch_profile_details_for_person(
    ...
    api_description = (
        "Profile Details API (Action 7)"  # Use specific description for context
    )
    profile_url = urljoin(
        config_instance.BASE_URL,
        ...
        if profile_response and isinstance(profile_response, dict):
            ...
            if first_name_raw and isinstance(first_name_raw, str):
                ...
                if display_name_raw:
                    ...
                    result_data["first_name"] = (
                        None  # Set explicitly to None if no name found
                    )
            ...
            if last_login_str:
                try:
                    ...
                    result_data["last_logged_in_dt"] = (
                        last_logged_in_dt_aware  # Store aware datetime
                    )
                    ...
                    result_data["last_logged_in_dt"] = (
                        None  # Set to None on parsing error
                    )
            else:
    ...
    # Step 5: Handle exceptions during the process
        ...
        requests.exceptions.RequestException
    ) as req_e:  # Catch other requests errors for retry
        logger.error(
...
def handle_twoFA(session_manager: SessionManager) -> bool:
    ...
    element_wait = selenium_config.element_wait(driver)
    page_wait = selenium_config.page_wait(
        driver
    )  # Not strictly used here, but available
...
def log_in(session_manager: SessionManager) -> str:
    ...
    try:
        ...
        if not enter_creds(driver):
            ...
            try:
                ...
                generic_alert_selector = (
                    "div.alert[role='alert']"  # Common pattern for alerts
                )
                try:
        ...
        try:
            ...
            if status is True:
                return "LOGIN_SUCCEEDED"  # Logged in despite error
            if status is False:
                return "LOGIN_FAILED_UNKNOWN"  # Not logged in
...
def close_tabs(driver: WebDriver):
    ...
    try:
        ...
        for handle in handles[1:]:
            try:
                ...
                logger.warning(
                    f"Tab {handle} already closed or could not be switched to."
                )
            except WebDriverException as e:
    ...
    # Step 6: Handle exceptions during the process
        ...
        logger.warning(
            "Attempted to close/switch to a tab that no longer exists during cleanup."
        )
    except WebDriverException as e:
                ...
                if session_manager:
                    ...
                    if login_stat is True:
                        ...
                        if login_result == "LOGIN_SUCCEEDED":
                            ...
                            logger.error(
                                f"Re-login attempt failed ({login_result}). Cannot complete navigation."
                            )
                            return False
                else:
                    logger.error(
            ...
            if landed_url_base != target_url_base:
                ...
                if is_signin_to_base_redirect:
                    ...
                    if session_manager and login_status(session_manager) is True:
                        ...
                        return (
                            True  # Treat this specific redirect as success if logged in
                        )
                ...
                if action == "skip":
                    return False  # Page no longer available
                if action == "refresh":
        ...
        # --- Step 6: Handle Exceptions During the Attempt ---
            ...
            if session_manager and not is_browser_open(driver):
                ...
                if session_manager.restart_sess():
                    ...
                    if not driver:
                        return False  # Ensure driver updated
                    continue  # Retry navigation attempt
                else:
...
# --- GEDCOM/Ancestry helpers (migrated from gedcom_interrogator.py) ---
import re
from datetime import datetime, timezone
...
def _get_full_name(indi) -> str:
    ...
    try:
        ...
        indi_id_log = (
            _normalize_id(indi.xref_id) if hasattr(indi, "xref_id") else "Unknown ID"
        )
        logger.error(f"Error formatting name for @{indi_id_log}@:", exc_info=False)
...
def _get_event_info(individual, event_tag: str) -> Tuple[Optional[datetime], str, str]:
    ...
    date_str: str = "N/A"
    place_str: str = "N/A"
    if _is_individual(individual) and hasattr(individual, "xref_id"):
        pass
    else:
...
def build_indi_index(reader):
    ...
    if "INDI_INDEX" not in globals():
        globals()["INDI_INDEX"] = {}
    if "INDI_INDEX_BUILD_TIME" not in globals():
        globals()["INDI_INDEX_BUILD_TIME"] = 0
    if INDI_INDEX:
        return
    import time
...
def build_family_maps(reader):
    ...
    for fam in reader.records0("FAM"):
        ...
        parents = set()
        if _is_individual(husband) and hasattr(husband, "xref_id") and husband.xref_id:
            parent_id_h = _normalize_id(husband.xref_id)
            if parent_id_h:
                parents.add(parent_id_h)
        if _is_individual(wife) and hasattr(wife, "xref_id") and wife.xref_id:
...
def _find_family_records_where_individual_is_parent(reader, target_id):
    parent_families = []
    for family_record in reader.records0("FAM"):
        if not _is_record(family_record) or not hasattr(family_record, "xref_id"):
            continue
...
def get_related_individuals(reader, individual, relationship_type: str) -> List:
    ...
    if (
        not reader
        or not _is_individual(individual)
...
def main():
    ...
    try:
        ...
        if not ready_ok or not session_manager.session_ready:
            logger.error(
                "SessionManager.ensure_session_ready() (Phase 2) FAILED. Aborting."
            )
            test_success = False
            return
        else:
        ...
        if config_instance.TREE_NAME and not session_manager.my_tree_id:
            errors.append(
                f"my_tree_id (required for TREE_NAME '{config_instance.TREE_NAME}')"
            )
