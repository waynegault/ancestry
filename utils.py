...
# NOTE: Database imports might be needed if utils directly interacts with models beyond SessionManager
...
# ------------------------------------------------------------------------------------
# Helper functions (Generic Utilities & Header Makers)
# ------------------------------------------------------------------------------------
...
def get_tot_page(driver: WebDriver) -> int:
    ...
    try:
        # Use selenium_config for the selector
        pagination_selector = getattr(selenium_config, "PAGINATION_SELECTOR", PAGINATION_SELECTOR) # Fallback to my_selectors just in case
        ...
        if pagination_element:
            ...
            if total_str and total_str.isdigit():
                return int(total_str)
                 ...
                 logger.warning(f"Could not parse total page count from attribute: '{total_str}'")
                 return 1 # Default if attribute invalid
        else:
            logger.debug("Pagination element found but was None. Defaulting to 1 page.")
            return 1
    except TimeoutException:
        logger.debug(
            f"Timeout waiting for pagination element '{pagination_selector}'. Assuming 1 page."
        )
        return 1
    except (NoSuchElementException, AttributeError): # Catch attribute error too
        logger.debug(
            f"Pagination element or 'total' attribute not found. Assuming 1 page."
        )
        return 1
    except Exception as e:
...
def ordinal_case(text: str) -> str:
    ...
    def lower_suffix(match: re.Match) -> str:
        ...
        # Ensure groups exist before accessing
        num_part = match.group(1) if match.group(1) else ""
        suffix_part = match.group(2) if match.group(2) else ""
        return num_part + suffix_part.lower()
    # End of lower_suffix
    ...
    try:
        return re.sub(r"(\d+)(st|nd|rd|th)", lower_suffix, text, flags=re.IGNORECASE)
         ...
         logger.error(f"Error applying ordinal_case regex to '{text}': {re_err}")
...
def format_name(name: Optional[str]) -> str:
    ...
    Also removes GEDCOM-style surname slashes.
    ...
    try:
        # Remove leading/trailing slashes common in GEDCOM names
        ...
        # Final check if formatting resulted in empty string
    ...
    try:
        ...
        newrelic_data = {
            ...
            "d": {
                ...
                "id": span_id, "tr": trace_id, "ti": int(time.time() * 1000), "tk": tk,
            },
        }
...
def retry(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            # This part should only be reached if attempts is 0 or loop fails unexpectedly
            logger.error(f"Function '{func.__name__}' did not return successfully (exited retry loop). Last Exception: {last_exception}")
            raise last_exception if last_exception else RuntimeError(f"Function '{func.__name__}' failed after {attempts} retries.")
...
def retry_api(
    ...
    retry_on_exceptions: Tuple[Type[Exception], ...] = (
        requests.exceptions.RequestException, ConnectionError # Added ConnectionError
    ),
    ...
    Uses defaults from config_instance if arguments are None. Handles ConnectionError.
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            # Ensure retry codes are sourced correctly from config if not provided
            _retry_codes_list = (
                config_instance.RETRY_STATUS_CODES
                if retry_on_status_codes is None
                else tuple(retry_on_status_codes) # Convert list to tuple if provided
            )
            _retry_codes_set = set(_retry_codes_list) # Use set for efficient lookup

            # Step 2: Initialize retry state variables
            retries_left = _max_retries
            current_delay = _initial_delay
                ...
                try:
                    ...
                    if response is not None and hasattr(response, "status_code"):
                        ...
                        if status_code in _retry_codes_set:
                            raise HTTPError(f"Retryable Status Code {status_code}", response=response) # Raise HTTPError to trigger retry logic

                    # Step 6: If status code doesn't trigger retry, return successful response
                    return response

                # Step 7: Handle exception-based retry condition (includes HTTPError raised above)
                except retry_on_exceptions as e:
                    last_exception = e
                    retries_left -= 1
                    if retries_left < 0: # Corrected condition (was <= 0)
                        logger.error(
                            f"API Call '{func.__name__}' failed after {attempt} attempts. Final Exception: {type(e).__name__} - {e}",
                            exc_info=False, # Less verbose for final retry failure
                        )
                        # Re-raise the *last caught exception* for the caller to handle
                        raise last_exception
                    else:
                        # Calculate sleep time with jitter
                        sleep_time = min(current_delay, config_instance.MAX_DELAY) + random.uniform(0, 0.2)
                        sleep_time = max(0.1, sleep_time) # Ensure minimum sleep

                        # Log warning with specific exception type
                        logger.warning(
                            f"API Call '{func.__name__}' failed (Attempt {attempt}/{_max_retries}) with {type(e).__name__}. Retrying in {sleep_time:.2f}s..."
                        )
                        time.sleep(sleep_time)
                        current_delay *= _backoff_factor # Increase delay for next potential retry
                        continue # Go to the next attempt

                # Step 8: Handle non-retryable exceptions (let them propagate)
                # No explicit catch needed here; they will exit the loop and function

            # Should not be reached unless initial max_retries = 0 or loop logic error
            logger.error(f"Exited retry loop unexpectedly for '{func.__name__}'. Last Exception: {last_exception}")
            # Re-raise the last known exception if one occurred, otherwise raise a generic error
            if last_exception:
                raise last_exception
            else:
                raise RuntimeError(f"Retry loop failed unexpectedly for '{func.__name__}'")
...
def ensure_browser_open(func: Callable) -> Callable:
    ...
    def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        # Step 3: Check if the browser session is valid using the SessionManager method
        if not session_manager_instance.is_sess_valid(): # Use the SM's own check
...
class DynamicRateLimiter:
    ...
    def __init__(
        ...
        self.backoff_factor = max(1.0, self.backoff_factor)
        self.decrease_factor = min(1.0, max(0.0, self.decrease_factor))
        ...
        # Ensure capacity and fill_rate are positive
        self.capacity = max(1.0, self.capacity) # Need at least 1 token capacity
        if self.fill_rate <= 0:
            logger.warning(f"Token fill rate ({self.fill_rate}) must be positive. Setting to 1.0.")
            self.fill_rate = 1.0
    ...
    def wait(self) -> float:
        ...
        if self.tokens < required_tokens:
            needed = required_tokens - self.tokens
            wait_for_token = needed / self.fill_rate
            # logger.debug(f"Token bucket low ({self.tokens:.2f}). Need to wait {wait_for_token:.3f}s for token refill.")

        # --- Calculate Base Delay ---
        # Apply the current dynamic delay value.
        base_dynamic_delay = self.current_delay

        # --- Combine Waits and Apply Jitter ---
        # Total wait is the max of time needed for token refill OR the current dynamic delay
        combined_wait = max(wait_for_token, base_dynamic_delay)
        # Apply jitter (+/- 10% of the combined wait)
        jitter = random.uniform(-0.1, 0.1) * combined_wait
        sleep_duration = combined_wait + jitter
        # Enforce min/max bounds
        sleep_duration = min(max(0.01, sleep_duration), self.MAX_DELAY) # Ensure 0.01s minimum, cap at MAX_DELAY

        # --- Perform Sleep and Consume Token ---
        if sleep_duration > 0.01: # Only sleep if duration is meaningful
            # logger.debug(f"Rate Limiter: Sleeping for {sleep_duration:.3f}s (WaitToken: {wait_for_token:.3f}s, BaseDelay: {base_dynamic_delay:.3f}s, Jitter: {jitter:.3f}s)")
            time.sleep(sleep_duration)
            # Refill again *after* sleeping to accurately account for time passed
            self._refill_tokens()

        # Consume the required tokens (even if we didn't wait, or waited less than needed)
        # Clamp tokens at 0 minimum.
        self.tokens = max(0.0, self.tokens - required_tokens)
        # logger.debug(f"Consumed {required_tokens} token(s). Tokens remaining: {self.tokens:.2f}")
    ...
    def reset_delay(self):
        ...
        if self.current_delay != self.initial_delay:
            ...
            logger.info(f"Rate limiter base delay reset to initial: {self.initial_delay:.2f}s")
    ...
    def decrease_delay(self):
        ...
        if not self.last_throttled and self.current_delay > self.initial_delay:
            ...
            if abs(previous_delay - self.current_delay) > 0.01: # Log only noticeable changes
                logger.debug(f"Decreased rate limit base delay to {self.current_delay:.2f}s")
        # Step 4: Reset throttle flag after any successful action (decrease or no change)
        # This ensures that after one successful non-throttled call, the next call can decrease again.
        self.last_throttled = False
    ...
    def increase_delay(self):
        ...
        # Added check to log only if delay actually increased
        if self.current_delay > previous_delay:
             logger.info(f"Rate limit feedback received. Increased base delay from {previous_delay:.2f}s to {self.current_delay:.2f}s")
        else:
             logger.info(f"Rate limit feedback received, but delay already at MAX ({self.MAX_DELAY:.2f}s).")
...
class SessionManager:
    ...
    def __init__(self):
        ...
        self.driver_live: bool = False # Indicates if driver is initialized and on base URL
        self.session_ready: bool = False # Indicates if driver is live, user logged in, identifiers fetched
        ...
        self.scraper: Optional[cloudscraper.CloudScraper] = None # Initialize as None first
        try:
             # Use defaults or configure as needed
             self.scraper = cloudscraper.create_scraper(
                 browser={'browser': 'chrome', 'platform': 'windows', 'desktop': True},
                 delay=5 # Reduced delay slightly, adjust as needed
             )
             scraper_retry = Retry(total=3, backoff_factor=0.8, status_forcelist=[403, 429, 500, 502, 503, 504])
             ...
             logger.debug("Initialized shared cloudscraper instance with retry strategy.")
        except Exception as scraper_init_e:
             logger.error(f"Failed to initialize cloudscraper instance: {scraper_init_e}", exc_info=True)
        ...
        self.last_js_error_check: datetime = datetime.now(timezone.utc) # Ensure timezone aware
    ...
    def _api_req(
        ...
        """
        V1.7 REVISED: Uses cloudscraper. Passes cookies via Cookie header.
        Temporarily removed tracing headers (newrelic, traceparent, tracestate) for testing.
        Handles dynamic header generation, rate limiting, retries, response processing.
        """
        # Use self attributes instead of arguments
        driver = self.driver
        session_manager = self

        # --- Prerequisite Checks ---
        if not session_manager.scraper:
            logger.error(f"{api_description}: Aborting - cloudscraper instance missing.")
            return None
        if driver and not self.is_sess_valid():
        ...
        # --- Header Preparation (Tracing Headers Commented Out) ---
        final_headers: Dict[str, str] = {}

        # 3a. Set Common Defaults / Dynamically Generated Headers
        ua = None
        if driver and self.is_sess_valid():
            try: ua = driver.execute_script("return navigator.userAgent;")
            except Exception: pass
        final_headers["User-Agent"] = ua or getattr(self.scraper, 'headers', {}).get('User-Agent', random.choice(config_instance.USER_AGENTS))
        ...
        if add_default_origin and http_method not in ["GET", "HEAD", "OPTIONS"]:
            try:
                ...
                logger.warning(f"Could not parse BASE_URL to set Origin header: {parse_err}")
        ...
        # --- ***** TEMPORARILY COMMENT OUT TRACING HEADERS ***** ---
        # logger.debug("Temporarily skipping newrelic, traceparent, tracestate headers for testing.")
        ...
        final_headers = {k: v for k, v in final_headers.items() if v} # Filter empty
        ...
        if self.my_profile_id:
            final_headers["ancestry-userid"] = self.my_profile_id.upper()

        if use_csrf_token:
            csrf_token = self.csrf_token
            if csrf_token: final_headers["X-CSRF-Token"] = csrf_token
            else: logger.warning(f"{api_description}: CSRF token requested but not found.")

        # 3b. Apply Headers Specific to API Description (Excluding Cookie)
        contextual_headers = config_instance.API_CONTEXTUAL_HEADERS.get(api_description, {})
        for key, value in contextual_headers.items():
             if key.lower() != 'cookie':
                 if value is not None and key not in final_headers: final_headers[key] = value
                 elif key == "ancestry-userid" and value is None and self.my_profile_id:
                      final_headers["ancestry-userid"] = self.my_profile_id.upper()

        # 3c. Apply Explicit Overrides passed via 'headers' argument (Excluding Cookie)
        if headers:
            filtered_overrides = {k: v for k, v in headers.items() if v is not None and k.lower() != 'cookie'}
            final_headers.update(filtered_overrides)

        # --- Special Handling for "Match List API" ---
        if api_description == "Match List API":
            final_headers.pop("Origin", None)
            final_headers.setdefault("Cache-Control", "no-cache")
            final_headers.setdefault("Pragma", "no-cache")
        ...
        req_session = self.scraper # Use cloudscraper
        if not req_session:
             logger.error(f"{api_description}: Aborting - cloudscraper instance unavailable.")
             return None
        ...
        while retries_left >= 0:
            ...
            attempt_headers = final_headers.copy() # Copy base headers for this attempt
            cookie_header_value: Optional[str] = None

            try:
                # --- Prepare Cookie Header String before each attempt ---
                if driver and self.is_sess_valid():
                    try:
                        driver_cookies_list = driver.get_cookies()
                        cookie_parts = [f"{c['name']}={c['value']}" for c in driver_cookies_list if 'name' in c and 'value' in c]
                        if cookie_parts:
                             cookie_header_value = "; ".join(cookie_parts)
                             attempt_headers['Cookie'] = cookie_header_value # Add/Override Cookie header
                             # logger.debug(f"Prepared Cookie header with {len(cookie_parts)} cookies for attempt {attempt}.")
                        else:
                             logger.warning(f"{api_description}: No cookies found in WebDriver for attempt {attempt}.")
                             attempt_headers.pop('Cookie', None) # Ensure no stale Cookie header
                    except Exception as cookie_err:
                        logger.warning(f"{api_description}: Error preparing Cookie header (Attempt {attempt}): {cookie_err}")
                        attempt_headers.pop('Cookie', None)
                else:
                    attempt_headers.pop('Cookie', None)
                # --- End Cookie Header Prep ---

                # Apply rate limit wait
                wait_time = self.dynamic_rate_limiter.wait()

                # Make the request using the scraper instance
                # Pass the headers including the Cookie header
                response = req_session.request(
                    method=http_method, url=url, headers=attempt_headers, data=data, json=json_data,
                    timeout=request_timeout, verify=True, allow_redirects=effective_allow_redirects
                    # No cookies= parameter needed here
                )
                ...
                if status in retry_status_codes:
                    ...
                    if retries_left <= 0:
                        logger.error(f"{api_description}: Failed after {attempt} attempts (Final Status {status}).")
                        return response # Return last error response
                    else:
                        sleep_time = min(current_delay, max_delay) + random.uniform(0, 0.2); sleep_time = max(0.1, sleep_time)
                        if status == 429: self.dynamic_rate_limiter.increase_delay()
                        logger.warning(f"{api_description}: Status {status} (Attempt {attempt}/{max_retries}). Retrying in {sleep_time:.2f}s...")
                        time.sleep(sleep_time); current_delay *= backoff_factor; retries_left -= 1
                        continue

                # Step 5b: Handle redirects when allow_redirects is False
                elif 300 <= status < 400 and not effective_allow_redirects:
                    logger.warning(f"{api_description}: Status {status} {response.reason} (Redirects Disabled). Returning response object.")
                    return response

                # Step 5c: Handle unexpected redirects when allow_redirects is True
                elif 300 <= status < 400 and effective_allow_redirects:
                    logger.warning(f"{api_description}: Unexpected final status {status} {response.reason} (Redirects Enabled). Returning response.")
                    return response

                # Step 5d: Process successful response (2xx)
                elif response.ok:
                    self.dynamic_rate_limiter.decrease_delay()
                    ...
                    if "application/json" in content_type:
                        ...
                        if csrf_text and not csrf_text.lower().startswith(('<!doctype html', '<html')): return csrf_text
                        else:
                           logger.warning(f"CSRF API returned text, but it might be HTML/empty: {csrf_text[:100]}")
                           return response # Return Response obj on likely failure
                    else:
                ...
                # Step 5e: Handle non-retryable client/server errors (4xx/5xx)
                    ...
                    if status in [401, 403]:
                        logger.warning(f"{api_description}: API call failed {status} {response.reason}. Possible Cloudflare block or invalid Cookie header/session?")
                        try:
                             if "cloudflare" in response.text.lower(): logger.warning("Cloudflare indicators detected in error response.")
                        except Exception: pass
                    else:
                        logger.error(f"{api_description}: Non-retryable error: {status} {response.reason}.")
                    try: logger.debug(f"Error Response Body: {response.text[:500]}")
                    except Exception: pass
                    return response # Return the error Response object

            # Step 6: Handle exceptions during the request attempt
            except (requests.exceptions.RequestException, ConnectionError) as e:
                last_exception = e; exception_type_name = type(e).__name__
                if retries_left <= 0:
                    logger.error(f"{api_description}: {exception_type_name} failed after {attempt} attempts. Error: {e}", exc_info=False)
                    return None
                else:
                    sleep_time = min(current_delay, max_delay) + random.uniform(0, 0.2); sleep_time = max(0.1, sleep_time)
                    logger.warning(f"{api_description}: {exception_type_name} (Attempt {attempt}/{max_retries}). Retrying in {sleep_time:.2f}s... Error: {e}")
                    time.sleep(sleep_time); current_delay *= backoff_factor; retries_left -= 1
                    continue
            except Exception as e:
        ...
        # --- End Retry Loop ---
        logger.error(f"{api_description}: Exited retry loop unexpectedly. Last Exception: {last_exception}.")
        return None
    # End of _api_req
    ...
    def start_sess(self, action_name: Optional[str] = None) -> bool:
        ...
        """
        logger.debug(f"--- SessionManager Phase 1: Starting Driver ({action_name or 'Unknown Action'}) ---")
        ...
        if not self.engine or not self.Session:
            try:
                ...
                logger.critical(f"DB Initialization failed during Phase 1 start: {db_init_e}")
        ...
        if not hasattr(self, '_requests_session') or not isinstance(self._requests_session, requests.Session):
             ...
             logger.debug("Shared requests.Session initialized (fallback in start_sess).")
        ...
        try:
            ...
            if not self.driver:
                 logger.error("WebDriver initialization failed (init_webdvr returned None after retries).")
            ...
            logger.debug(f"Navigating to Base URL ({config_instance.BASE_URL}) to stabilize...")
            # --- Ensure this call matches the function definition ---
            base_url_nav_ok = nav_to_page(
                 self.driver, config_instance.BASE_URL,
            ...
            self.last_js_error_check = datetime.now(timezone.utc) # Ensure timezone aware
        ...
        # Step 8: Handle exceptions during driver start or initial navigation
            ...
            logger.error(f"WebDriverException during Phase 1 start/base nav: {wd_exc}", exc_info=False)
            self.close_sess() # Clean up
            return False
        except Exception as e:
            logger.error(f"Unexpected error during Phase 1 start/base nav: {e}", exc_info=True)
            self.close_sess() # Clean up
            return False
    # End of start_sess

    def ensure_driver_live(self, action_name: Optional[str] = "Ensure Driver Live") -> bool:
        """
        ...
        """
        if self.driver_live and self.is_sess_valid(): # Add validity check here
            logger.debug(f"Driver already live and valid (Action: {action_name}).")
            return True
        else:
            logger.debug(f"Driver not live or invalid, attempting start (Action: {action_name})...")
    ...
    def _perform_readiness_checks(self, action_name: Optional[str] = None) -> bool:
        ...
        """
        logger.debug(f"--- SessionManager Performing Readiness Checks ({action_name or 'Unknown Action'}) ---")

        # Step 1: Prerequisite check (driver must be live and valid)
        if not self.driver_live or not self.is_sess_valid(): # Check validity too
            logger.error("Cannot perform readiness checks: Driver not live or invalid.")
            return False

        # Step 2: Perform readiness checks with limited retries for this phase
        ...
        for attempt in range(1, max_readiness_attempts + 1):
            logger.debug(f"Internal readiness check attempt {attempt}/{max_readiness_attempts}...")
            try:
                # Step 2a: Login Status Check & Attempt
                ...
                if login_stat is True:
                    ...
                    if login_result != "LOGIN_SUCCEEDED":
                        logger.error(f"Login attempt failed ({login_result}). Readiness check failed.")
                        return False # Fail readiness if login fails
                    logger.info("Login successful.")
                    # Re-verify status after login attempt
                    time.sleep(1) # Short pause before re-check
                    if not login_status(self):
                        logger.error("Login status verification failed after successful login report.")
                        return False
                ...
                # Step 2b: URL Check/Handling (Navigate to base URL if on signin/MFA etc.)
                ...
                # Step 2c: Essential Cookies Check
                logger.debug("Verifying essential cookies...")
                essential_cookies = ['ANCSESSIONID'] # SecureATT might not always be present
                if not self.get_cookies(essential_cookies, timeout=10): # Reduced timeout
                    logger.warning(f"Essential cookies {essential_cookies} not found. Proceeding cautiously.")
                    # Don't fail immediately, some APIs might work without all cookies
                else:
                    logger.debug("Essential cookies OK.")

                # Step 2d: Cookie Sync (WebDriver -> requests.Session)
                logger.debug("Syncing cookies...")
                self._sync_cookies()
                logger.debug("Cookies synced.")

                # Step 2e: CSRF Token (Fetch if missing/short)
                logger.debug("Ensuring CSRF token...")
                if not self.csrf_token or len(self.csrf_token) < 10: # Basic check
                ...
                # Step 2f: Identifiers (Profile ID, UUID, Tree ID)
                logger.debug("Retrieving identifiers...")
                if not self._retrieve_identifiers(): # Handles logging errors internally
                    logger.error("Failed to retrieve essential identifiers.")
                    return False
                logger.debug("Identifiers retrieved OK.")

                # Step 2g: Tree Owner Name (If tree ID exists)
                ...
                # Step 2h: Mark Ready & Success
                self.session_ready = True # Set the flag
                logger.debug(f"--- SessionManager Readiness Checks Passed ({action_name or 'Unknown Action'}) ---")
                return True # All checks passed for this attempt

            # --- Handle Exceptions during the attempt ---
            except WebDriverException as wd_exc:
                 logger.error(f"WebDriverException during internal readiness check attempt {attempt}: {wd_exc}", exc_info=False)
                 ...
                 if attempt >= max_readiness_attempts:
                      logger.error("Readiness checks failed after final attempt (WebDriverException).")
                 ...
                 logger.error(f"Unexpected error during internal readiness check attempt {attempt}: {e}", exc_info=True)
                 if attempt >= max_readiness_attempts:
                      logger.error("Readiness checks failed after final attempt (Exception).")
                      return False
                 logger.info(f"Waiting {self.selenium_config.CHROME_RETRY_DELAY}s before next readiness attempt...")
                 time.sleep(self.selenium_config.CHROME_RETRY_DELAY)
                 # Continue to next attempt in the loop
        ...
        # --- End of Retry Loop ---
        logger.error(f"Internal readiness checks FAILED after {max_readiness_attempts} attempts.")
    ...
    def ensure_session_ready(self, action_name: Optional[str] = None) -> bool:
        ...
        return self._perform_readiness_checks(action_name=f"{action_name} - Readiness Checks")
    ...
    def _initialize_db_engine_and_session(self):
        ...
        if self.engine and self.Session:
            logger.debug(f"DB Engine/Session already initialized for SM ID={id(self)}. Skipping.")
            return

        logger.debug(f"SessionManager ID={id(self)} initializing SQLAlchemy Engine/Session...")
        self._db_init_attempted = True # Mark that we tried

        # Step 2: Dispose existing engine if present (e.g., during restart)
        if self.engine:
            logger.debug(f"Disposing existing engine before re-initializing (SM ID={id(self)}).")
            try: self.engine.dispose()
            except Exception as dispose_e: logger.error(f"Error disposing existing engine: {dispose_e}")
            self.engine = None
            self.Session = None
        ...
        try:
            ...
            pool_size_config = getattr(config_instance, "DB_POOL_SIZE", 5) # Default 5
            pool_size = 5 # Fallback default
            try:
                parsed_val = int(pool_size_config)
                pool_size = max(1, min(parsed_val, 100)) # Ensure 1 <= size <= 100
                if parsed_val != pool_size:
                    logger.warning(f"DB_POOL_SIZE adjusted from {parsed_val} to {pool_size}.")
            except (ValueError, TypeError):
                logger.warning(f"Could not parse DB_POOL_SIZE '{pool_size_config}'. Using default {pool_size}.")

            max_overflow = max(2, int(pool_size * 0.2)) # At least 2 overflow
            pool_timeout = 30
            pool_class = sqlalchemy_pool.QueuePool
            logger.debug(f"DB Pool Config: Size={pool_size}, MaxOverflow={max_overflow}, Timeout={pool_timeout}")
            ...
            logger.debug(f"Created NEW SQLAlchemy engine: ID={id(self.engine)} for SM ID={id(self)}")
            ...
            def enable_sqlite_settings(dbapi_connection, connection_record):
                 ...
                 try:
                      ...
                      # logger.debug("SQLite PRAGMA settings applied (WAL, Foreign Keys).") # Can be noisy
            ...
            try:
                 ...
                 logger.error(f"Error creating DB tables: {table_create_e}", exc_info=True)
    ...
    def get_db_conn(self) -> Optional[Session]:
        ...
        """
        # Step 1: Ensure DB engine/session factory is initialized
        if not self.engine or not self.Session:
        ...
        # Step 2: Attempt to get a session from the factory
        try:
            # self.Session should now be a valid sessionmaker factory
            new_session = self.Session()
            # logger.debug(f"SessionManager ID={id(self)} obtained DB session {id(new_session)} from Engine ID={id(self.engine)}")
            return new_session
        except Exception as e:
            # Step 3: Handle errors during session creation from the factory
            ...
            if self.engine:
                try: self.engine.dispose()
                except Exception: pass # Ignore errors during disposal here
            self.engine = None
            self.Session = None
    ...
    def get_db_conn_context(self) -> Generator[Optional[Session], None, None]:
        ...
        try:
            ...
            if session:
                ...
                if session.is_active:
                    # logger.debug(f"DB Context Manager: Committing session {session_id_for_log}.")
                    session.commit()
                    # logger.debug(f"DB Context Manager: Commit successful for session {session_id_for_log}.")
                else:
                    logger.warning(f"DB Context Manager: Session {session_id_for_log} inactive after yield, skipping commit.")
            else:
        ...
        # Step 4: Handle exceptions raised within the 'with' block or during commit
            ...
            logger.error(f"DB Context Manager: Exception occurred ({type(e).__name__}). Rolling back session {session_id_for_log}.", exc_info=True)
            if session and session.is_active:
                try:
                    session.rollback()
                    logger.warning(f"DB Context Manager: Rollback successful for session {session_id_for_log}.")
                except Exception as rb_err:
                    logger.error(f"DB Context Manager: Error during rollback for session {session_id_for_log}: {rb_err}")
        ...
        # Step 5: Ensure session is returned to pool regardless of success/failure
             ...
             if session:
                  ...
                  # logger.debug(f"DB Context Manager: Returned session {session_id_for_log} to pool.")
             # else: logger.debug("DB Context Manager: No valid session to return.")
    ...
    def return_session(self, session: Session):
        ...
        # else: logger.warning("Attempted to return a None DB session.")
    ...
    def cls_db_conn(self, keep_db: bool = True):
        ...
        if keep_db:
             ...
             # logger.debug(f"cls_db_conn called (keep_db=True). Skipping engine disposal for Engine ID: {engine_id_str}")
             return
        ...
        logger.debug(f"SessionManager ID={id(self)} cls_db_conn called (keep_db=False). Disposing Engine ID: {engine_id}")
        try:
    ...
    # --- Identifier and State Retrieval ---
    ...
    def get_my_profileId(self) -> Optional[str]:
        ...
        try:
            response_data = self._api_req( # Use self._api_req
                url=url,
                method="GET",
            ...
            if not response_data:
                logger.warning("Failed to get profile_id response via _api_req (returned None).")
                return None
    ...
    def get_my_uuid(self) -> Optional[str]:
        ...
        response_data = self._api_req( # Use self._api_req
             url=url, method="GET",
             use_csrf_token=False, api_description="Get UUID API"
        )
        ...
        if response_data:
             ...
             logger.error("Failed to get header/dna data via _api_req (returned None or error response).")
             return None
    # End of get_my_uuid
    ...
    def get_my_tree_id(self) -> Optional[str]:
        ...
        logger.debug(f"Attempting to fetch tree ID for TREE_NAME='{tree_name_config}'...")

        # Step 4: Call API using helper
        try:
            response_data = self._api_req( # Use self._api_req
                url=url, method="GET",
                use_csrf_token=False, api_description="Header Trees API"
            )
    ...
    def get_tree_owner(self, tree_id: str) -> Optional[str]:
        ...
        try:
             response_data = self._api_req( # Use self._api_req
                  url=url, method="GET",
                  use_csrf_token=False, api_description="Tree Owner Name API"
             )
             ...
             if response_data and isinstance(response_data, dict):
                  ...
                  if owner_data and isinstance(owner_data, dict):
                       ...
                       if display_name and isinstance(display_name, str):
                            ...
                            return display_name
                       else: logger.warning(f"Could not find 'displayName' in owner data for tree {tree_id}.")
                  else: logger.warning(f"Could not find 'owner' data in Tree Owner API response for tree {tree_id}.")
        ...
        # Step 6: Handle exceptions
             ...
             logger.error(f"Error fetching/parsing Tree Owner API for tree {tree_id}: {e}", exc_info=True)
             return None
    ...
    def get_csrf(self) -> Optional[str]:
        ...
        # Step 2: Call the API using _api_req helper
        response_data = self._api_req( # Use self._api_req
            url=csrf_token_url,
            method="GET",
        ...
        if response_data and isinstance(response_data, str):
            ...
            logger.warning("Failed to get CSRF token response via _api_req (returned None).")
            return None
        else:
            # Unexpected response format
            logger.error(f"Unexpected response type for CSRF token API: {type(response_data)}")
    ...
    def get_cookies(self, cookie_names: List[str], timeout: int = 10) -> bool:
        ...
        while time.time() - start_time < timeout:
             try:
                  ...
                  current_cookies_lower = {c['name'].lower() for c in cookies if 'name' in c} # Handle potential malformed cookies
             ...
             # Step 3: Handle exceptions during polling
                  ...
                  # time.sleep(interval * 2) # Removed retry here, fail faster
             except Exception as e:
                  logger.error(f"Unexpected error retrieving cookies: {e}", exc_info=True)
                  # Decide policy: fail fast
                  return False
        ...
        try:
            ...
            else: missing_final = cookie_names # Assume all missing if driver gone/invalid
        except Exception: missing_final = cookie_names # Assume all missing on error
    ...
    def verify_sess(self) -> bool:
        """DEPRECATED: Use login_status() or ensure_session_ready()."""
        logger.warning("verify_sess() is deprecated. Use login_status() or ensure_session_ready().")
        # Maintain old behavior for compatibility for now
        if not self.is_sess_valid(): return False
        if not self._validate_sess_cookies(['ANCSESSIONID']): return False # Basic cookie check
        # Simple check for logged-in element as a proxy
        try:
            WebDriverWait(self.driver, 1).until(EC.presence_of_element_located((By.CSS_SELECTOR, CONFIRMED_LOGGED_IN_SELECTOR)))
            return True
    ...
    def is_sess_valid(self) -> bool:
        ...
        # Step 4: Handle specific exceptions indicating invalid session
            ...
            return False
        except (NoSuchWindowException, WebDriverException) as e: # Catch other relevant WebDriver errors
            err_str = str(e).lower()
            # Check for common error messages indicating a dead session
            if "disconnected" in err_str or "target crashed" in err_str or "no such window" in err_str or "unable to connect" in err_str:
    ...
    def close_sess(self, keep_db: bool = False):
        ...
        # else: logger.debug("No active WebDriver session to close.")
        ...
        # else: logger.debug("Keeping DB connection pool alive (keep_db=True).")
    # End of close_sess
    ...
    def restart_sess(self, url: Optional[str] = None) -> bool:
        ...
        if url and self.driver: # Check driver exists again after successful ready phase
             ...
             if nav_to_page(self.driver, url, selector="body", session_manager=self):
                  ...
                  logger.error(f"Failed to re-navigate to {url} after successful restart.")
                  return False # Navigation failed
        elif not url:
             logger.info("Session restart successful (no navigation requested).")
             return True # Success (restart only)
        else: # Should not happen if ready_ok is True, but safeguard
    ...
    def make_tab(self) -> Optional[str]:
        ...
        try:
            ...
            if new_tab_handles:
                 ...
                 logger.error("Could not identify new tab handle (set difference empty).")
        ...
        # Step 5: Handle timeout or errors during wait/identification
            ...
            try: logger.debug(f"Window handles during timeout: {driver.window_handles}")
            except Exception: pass
            return None
        except (IndexError, WebDriverException) as e:
            logger.error(f"Error identifying new tab handle after wait: {e}")
            try: logger.debug(f"Window handles during error: {driver.window_handles}")
            except Exception: pass
            return None
        except Exception as e:
    ...
    def _check_and_handle_url(self) -> bool:
        ...
        try:
             ...
             if not self.is_sess_valid(): logger.warning("Session seems invalid during URL check.")
             return False
        ...
        if needs_navigation:
             logger.info(f"Current URL unsuitable ({reason}). Navigating to base URL: {base_url_norm}")
        ...
        # else: logger.debug("Current URL is suitable, no extra navigation needed.")
    ...
    def _retrieve_identifiers(self) -> bool:
        ...
        if config_instance.TREE_NAME and not self.my_tree_id: # Only fetch if needed and not set
             logger.debug(f"Retrieving tree ID for tree name: '{config_instance.TREE_NAME}'...")
    ...
    def _retrieve_tree_owner(self):
        ...
        if self.my_tree_id and not self.tree_owner_name:
             logger.debug(f"Retrieving tree owner name for tree ID: {self.my_tree_id}...")
             ...
             elif not self.tree_owner_name: logger.warning("Failed to retrieve tree owner name.\n")
    ...
    def _sync_cookies(self):
        ...
        try:
             driver_cookies = self.driver.get_cookies() if self.driver else []
             ...
             if not self.is_sess_valid(): logger.error("Session invalid after WebDriverException during cookie sync.")
             return
        except Exception as e:
        ...
        for cookie in driver_cookies:
            ...
            if not isinstance(cookie, dict) or 'name' not in cookie or 'value' not in cookie or 'domain' not in cookie:
            ...
            try:
                 ...
                 # Ensure domain starts with '.' if it's not localhost or IP
                 if not domain_to_set.startswith('.') and domain_to_set not in ['localhost'] and not re.match(r'^\d+\.\d+\.\d+\.\d+$', domain_to_set):
                      domain_to_set = '.' + domain_to_set
    ...
    def _validate_sess_cookies(self, required_cookies: List[str]) -> bool:
        ...
        try:
             ...
             if not missing_cookies: return True # All found
             else:
        ...
        # Step 4: Handle exceptions during cookie retrieval
             ...
             # Check session died after exception
             if not self.is_sess_valid(): logger.error("Session invalid after WebDriverException during cookie validation.")
             return False
        except Exception as e:
    ...
    def _verify_api_login_status(self) -> Optional[bool]:
        ...
        api_description = "API Login Verification (header/dna)" # Corrected description lookup key
        ...
        try:
            ...
            if response_data is None:
                # logger.warning(f"{api_description}: _api_req returned None. Returning None.")
                return None
            elif isinstance(response_data, requests.Response): # Error response object
                ...
                if "testId" in response_data: # Check for testId (UUID)
                    # logger.debug(f"{api_description}: API login check successful ('testId' found).")
                    return True
                else:
                    # Log unexpected format but cautiously assume logged in if status was 2xx
                    logger.warning(f"{api_description}: API check succeeded (2xx), but response format unexpected: {response_data}. Assuming logged in cautiously.")
                    return True
            else:
        ...
        # Step 6: Handle exceptions during the API check process itself
            ...
            logger.error(f"Unexpected error during {api_description}: {e}", exc_info=True)
            return None
    ...
    def check_js_errors(self):
        ...
        try:
            ...
            if "browser" not in log_types:
                 # logger.debug("Browser log type not supported.")
        ...
        try: logs = self.driver.get_log("browser")
        except WebDriverException as e:
        ...
        most_recent_error_time_this_check = self.last_js_error_check # Track latest error in *this* check
        ...
        for entry in logs:
            ...
            if isinstance(entry, dict) and entry.get("level") == "SEVERE":
                try:
                    ...
                    else: logger.warning(f"JS Log entry missing timestamp: {entry}")
        ...
        if new_errors_found:
             ...
             # logger.debug(f"Updated last_js_error_check time to: {self.last_js_error_check}")
...
def login_status(session_manager: SessionManager) -> Optional[bool]:
    ...
    try:
        # Check for absence of login button AND presence of logged-in indicator
        login_button_selector = LOG_IN_BUTTON_SELECTOR
        logged_in_selector = CONFIRMED_LOGGED_IN_SELECTOR

        # Check if logged-in element is present first (more definitive)
        ui_element_present = is_elem_there(driver, By.CSS_SELECTOR, logged_in_selector, wait=2) # Short wait

        if ui_element_present:
            logger.debug("Login status confirmed TRUE via UI check (logged-in element found).")
            return True
        else:
            # Logged-in element not found, now check if login button *is* present
            login_button_present = is_elem_there(driver, By.CSS_SELECTOR, login_button_selector, wait=1) # Very short wait
            if login_button_present:
                logger.debug("Login status confirmed FALSE via UI check (logged-in element absent, login button found).")
                return False
            else:
                # Ambiguous: Neither logged-in element nor login button found quickly.
                ...
                # Defaulting to False in ambiguous state is safer
                return False

    # Handle Exceptions during UI checks
    except WebDriverException as e:
...
def log_in(session_manager: SessionManager) -> str:
    ...
    # Step 1: Validate driver state
    driver = session_manager.driver
    if not driver or not session_manager.is_sess_valid(): # Check validity too
        logger.error("Login failed: WebDriver not available or session invalid.")
        ...
        try:
             ...
             if accept_button:
                  ...
                  # Step 4b: Handle click interception, try JS click
                       ...
                       except Exception as js_click_err: logger.error(f"Failed JS click for accept button: {js_click_err}")
                  # Step 4c: Handle other click errors
                  except Exception as click_err: logger.error(f"Error clicking accept button: {click_err}")
        # Step 4d: Handle errors finding/clicking the specific button
        except TimeoutException: logger.warning(f"Specific accept button '{consent_ACCEPT_BUTTON_SELECTOR}' not found or not clickable.")
        except Exception as find_err: logger.error(f"Error finding/clicking specific accept button: {find_err}")
...
def enter_creds(driver: WebDriver) -> bool:
    ...
    try:
        ...
        try: # Clear field robustly
            username_input.click(); time.sleep(0.2); username_input.clear(); time.sleep(0.2)
        except Exception as e:
        ...
        try: # Clear field robustly
            password_input.click(); time.sleep(0.2); password_input.clear(); time.sleep(0.2)
        except Exception as e:
        ...
        if sign_in_button:
             ...
             except ElementClickInterceptedException: logger.warning("Standard click intercepted. Trying JS click...")
             except ElementNotInteractableException as eni_e: logger.warning(f"Button not interactable for standard click: {eni_e}. Trying JS click...")
             except Exception as click_e: logger.error(f"Error during standard click: {click_e}. Trying JS click...")
             ...
             if not click_successful:
                  try:
                       ...
                       click_successful = True
                  except Exception as js_click_e: logger.error(f"Error during JavaScript click: {js_click_e}")
             ...
             if not click_successful:
                  ...
                  except Exception as key_e: logger.error(f"Failed to send RETURN key as final fallback: {key_e}")
...
def handle_twoFA(session_manager: SessionManager) -> bool:
    ...
    try:
        ...
        try:
             ...
             return False # Changed from True to False - if header missing and not logged in, it's a failure state
        ...
        try:
            ...
            if sms_button_clickable:
                ...
                except TimeoutException: logger.warning("Code input field did not appear/become visible after clicking 'Send Code'.")
                except Exception as e_input: logger.error(f"Error waiting for 2FA code input field: {e_input}. Check selector: {TWO_FA_CODE_INPUT_SELECTOR}")
            else:
...
def is_browser_open(driver: Optional[WebDriver]) -> bool:
    ...
    # Step 2: Attempt a lightweight WebDriver command
    try:
        _ = driver.window_handles # Accessing window_handles requires communication
        # Step 3: Return True if command succeeds
        return True
    ...
    try:
        ...
        for handle in handles[1:]:
             try:
                  ...
                  closed_count += 1
             except NoSuchWindowException: logger.warning(f"Tab {handle} already closed or could not be switched to.")
             except WebDriverException as e: logger.error(f"Error closing tab {handle}: {e}")
        ...
        if original_handle in remaining_handles:
            ...
            # else: logger.debug("Already focused on original tab.")
        ...
        else: logger.error("All browser tabs were closed unexpectedly.")

    # Step 6: Handle exceptions during the process
    except NoSuchWindowException: logger.warning("Attempted to close/switch to a tab that no longer exists during cleanup.")
    except WebDriverException as e:
         logger.error(f"WebDriverException during close_tabs: {e}")
         if not is_browser_open(driver): logger.error("Session invalid during close_tabs operation.")
    except Exception as e: logger.error(f"Unexpected error in close_tabs: {e}", exc_info=True)
# End of close_tabs
...
def nav_to_page(
    ...
    unavailability_selectors = {
         ...
         UNAVAILABLE_MATCH_SELECTOR: ("skip", 0), # Added for unavailable matches
         MESSAGE_CENTER_UNAVAILABLE_SELECTOR: ("skip", 0), # Added for unavailable message center
    }
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            is_on_mfa_page = is_elem_there(driver, By.CSS_SELECTOR, TWO_STEP_VERIFICATION_HEADER_SELECTOR, wait=1)
            if is_on_mfa_page:
            ...
            is_on_login_page = is_elem_there(driver, By.CSS_SELECTOR, USERNAME_INPUT_SELECTOR, wait=1)
            ...
            if landed_url_base != target_url_base:
                ...
                is_signin_to_base_redirect = (target_url_base == signin_page_url_base and landed_url_base == urlparse(config_instance.BASE_URL).path.rstrip('/'))
                ...
                if action == "refresh":
                     ...
                     continue # Temporary issue, retry
        ...
        # --- Step 6: Handle Exceptions During the Attempt ---
            ...
            alert_text = "N/A" # Default
            try:
                alert_text = alert_e.alert_text
            except Exception:
                pass # Ignore errors getting alert text
...
def main():
    ...
    try:
        ...
        if config_instance.TREE_NAME and not session_manager.my_tree_id: errors.append(f"my_tree_id (required for TREE_NAME '{config_instance.TREE_NAME}')")
        ...
        if not driver_instance:
            ...
            nav_ok = nav_to_page(
                driver=driver_instance, url=config_instance.BASE_URL,
                selector="body", session_manager=session_manager
            )
            if nav_ok:
        ...
        csrf_test_response = session_manager._api_req( # Call as method
             url=csrf_url, method="GET",
             use_csrf_token=False, api_description="CSRF Token API Test",
