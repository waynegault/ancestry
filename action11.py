...
# Removed: Module-level SessionManager creation. Use global session from session_utils.
    ...
    try:
        ...
        if GEDCOM_SCORING_AVAILABLE and scoring_func == calculate_match_score and scoring_func is not None:
            ...
            if "fraser" in candidate_data_dict.get("first_name", "").lower():
                logger.debug("=== ACTION 11 SCORING DEBUG ===")
                logger.debug(f"Search criteria: {search_criteria}")
                logger.debug(f"Candidate data: {candidate_data_dict}")
                logger.debug(f"Scoring weights: {scoring_weights}")
                logger.debug(f"Date flexibility: {date_flex}")
                logger.debug(f"Name flexibility: {name_flex}")
            ...
            if "fraser" in candidate_data_dict.get("first_name", "").lower():
                ...
                logger.debug("=== END ACTION 11 SCORING DEBUG ===")
...
def _call_direct_treesui_list_api(
    ...
    try:
        ...
        if cookies:
            logger.debug(f"API Request Cookies: {len(cookies)} keys")
...
def _format_tree_ladder_path(
    ...
    try:
        ...
        # Get raw formatted path (suppress full content in logs; log length only)
        raw_formatted_path = format_api_relationship_path(api_response_dict, on_str, sn_str)
        logger.debug(f"Raw path generated by format_api_relationship_path (len={len(raw_formatted_path)})")
...
def _check_dependencies() -> tuple[bool, list[str]]:
    ...
    dependencies = [
        ...
        # Session manager dependency: require a registered global session
        ("Global Session Registered", __import__('session_utils').get_global_session() is not None),
    ]
...
def _display_family_members_simple(family_data: dict) -> None:
    """Display family members without siblings section (Action 11 only)."""
    relation_labels = {
        "parents": "📋 Parents",
        "spouses": "💕 Spouses",
        "children": "👶 Children",
...
def _handle_supplementary_info_phase(
    ...
    if selected_person_tree_id and selected_tree_id:
        ...
        # Display empty family data (no siblings section for Action 11)
        print("📋 Parents:")
        print("   - None found")
        print("\n💕 Spouses:")
        print("   - None found")
...
def _init_and_auth_session(active_session_manager: SessionManager) -> bool:
    """Unify and authenticate the session using centralized session_utils.

    - Register the passed session as the global session for all modules
    - Ensure authentication via get_authenticated_session (handles browser + cookies)
    """
    try:
        # Import locally to avoid any potential import ordering issues
        from session_utils import get_authenticated_session, set_global_session  # type: ignore
        # Register globally so api_utils and others reuse the same instance
        set_global_session(active_session_manager)
        # This will start browser if needed, sync cookies, and verify login via API
        get_authenticated_session(action_name="Action 11 - API Report")
        return True
    except Exception as e:
        logger.error(f"Session authentication failed in _init_and_auth_session: {e}")
        print("\nERROR: Session authentication failed. Check logs.")
        return False
...
def handle_api_report(session_manager_param: Optional[Any] = None) -> bool:
    ...
    # Use the session_manager passed by the framework if available; otherwise use the global session
    if session_manager_param is not None:
        active_session_manager = session_manager_param
        logger.debug("Using session_manager passed by framework.")
    else:
        from session_utils import get_global_session  # type: ignore
        active_session_manager = get_global_session()
        if active_session_manager is None:
            logger.critical("No global session registered. main.py must register the global session before running Action 11.")
            print("\nCRITICAL: No global session registered. Please run main.py to register the global session, then retry Action 11.")
            return False
    ...
    # Session unification and authentication (single path)
    elif not _init_and_auth_session(active_session_manager):
...
def main() -> None:
    ...
    # Session is managed globally; handle_api_report() will validate/register usage
    try:
        report_successful = handle_api_report()
        duration = time.time() - start_ts
        if report_successful:
            logger.info(f"--- Action 11 finished successfully in {duration:.2f}s ---")
        else:
            error_message = f"Action 11 finished with errors in {duration:.2f}s"
...
def run_action11(session_manager_param: Any, *_: Any) -> bool:
    """Wrapper function for main.py to call. Requires a session manager from main.py."""
