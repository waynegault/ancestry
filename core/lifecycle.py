...
def _perform_proactive_cookie_sync(session_manager: SessionManager) -> None:
    """Perform proactive cookie sync during warmup."""
    try:
        ...
        if session_manager.is_sess_valid() and api_manager and browser_manager:
            logger.info("🔄 Performing proactive session refresh at startup...")

            # 1. Clear internal caches to force fresh validation
            # This prevents using stale cached session states that might cause 303 errors
            if hasattr(session_manager, "_clear_session_caches"):
                session_manager._clear_session_caches()
            if hasattr(session_manager, "_clear_last_readiness_check"):
                session_manager._clear_last_readiness_check()

            # 2. Force cookie sync
            logger.debug("Syncing browser cookies to API session during warmup...")
            # Use session_manager directly to pass force=True
            if hasattr(session_manager, "sync_browser_cookies"):
                synced = session_manager.sync_browser_cookies(force=True)
            else:
                # Fallback to API manager (no force param)
