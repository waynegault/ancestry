...
def _run_basic_fallback_tests() -> bool:
    ...
    # Test 1: API Manager creation
    try:
        api_manager = APIManager()
        if api_manager is not None:
            tests_passed += 1
            print("✅ API Manager creation test passed")
        else:
            print("❌ API Manager creation test failed")
    except Exception as e:
        print(f"❌ API Manager creation test error: {e}")

    # Test 2: Basic attributes
    try:
        api_manager = APIManager()
        required_attrs = ["csrf_token", "my_profile_id", "my_uuid"]

        attrs_found = 0
        for attr in required_attrs:
            if hasattr(api_manager, attr):
                attrs_found += 1

        if attrs_found >= 2:
            tests_passed += 1
            print("✅ Basic attributes test passed")
        else:
            print("❌ Basic attributes test failed")
    except Exception as e:
        print(f"❌ Basic attributes test error: {e}")

    # Test 3: Requests session
    try:
        api_manager = APIManager()
        session = api_manager.requests_session
        ...
        if session is not None and hasattr(session, "get"):
            tests_passed += 1
            print("✅ Requests session test passed")
        else:
            print("❌ Requests session test failed")
    except Exception as e:
        print(f"❌ Requests session test error: {e}")

    # Test 4: Essential identifiers check
    try:
        api_manager = APIManager()
        result = api_manager.has_essential_identifiers

        if isinstance(result, bool):
            tests_passed += 1
            print("✅ Essential identifiers check test passed")
        else:
            print("❌ Essential identifiers check test failed")
    except Exception as e:
        print(f"❌ Essential identifiers check test error: {e}")
    ...
    # Test 6: API constants
    try:
        constants = ["API_PATH_CSRF_TOKEN", "API_PATH_PROFILE_ID", "KEY_UCDMID"]
        constants_found = 0
        ...
        if constants_found >= 2:
            tests_passed += 1
            print("✅ API constants test passed")
        else:
            print("❌ API constants test failed")
    except Exception as e:
        print(f"❌ API constants test error: {e}")

    print(
        f"🏁 API Manager fallback tests completed: {tests_passed}/{tests_total} passed"
    )
