...
Automated checking for Python best practices violations throughout the
codebase, including function length, complexity, type hints, and SOLID principles.
...
Main Classes:
- QualityMetrics: Immutable dataclass for quality measurements
...
class CodeQualityChecker:
    ...
    def _find_type_ignore_violations(content: str) -> list[str]:
        ...
        for line_number, line in enumerate(content.splitlines(), start=1):
            ...
            if "type: ignore" in line[comment_index:].lower():
                ...
                violations.append(f"Line {line_number} uses 'type: ignore' (disallowed): {snippet}")
        return violations
    ...
    def _has_mutable_defaults(func: ast.FunctionDef) -> bool:
        ...
        return any(isinstance(default, (ast.List, ast.Dict, ast.Set)) for default in func.args.defaults)
...
def code_quality_checker_module_tests() -> bool:
    ...
    try:
        ...
        def test_type_ignore_detection() -> None:
            ...
            checker = CodeQualityChecker()
            # Construct the sample dynamically to avoid self-detection
            sample = "value = 1  # " + "type" + ": " + "ignore[attr-defined]"
...
# Use centralized test runner utility
run_comprehensive_tests = create_standard_test_runner(code_quality_checker_module_tests)
