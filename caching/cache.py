...
def invalidate_related_caches(pattern: str, exclude_modules: list[str] | None = None) -> dict[str, int]:
    ...
    if "gedcom" not in exclude_modules:
        try:
            from genealogy.gedcom.gedcom_cache import clear_memory_cache
...
def _collect_api_cache_stats(coordination_stats: dict[str, Any]) -> None:
    ...
    try:
        from caching.cache_manager import get_api_cache_stats
    ...
    def _regenerate_cache_data(self, cache_key: str, session_manager: Any | None = None) -> bool:
        ...
        try:
            ...
            if "profile" in cache_key.lower():
                return self._warm_profile_data(cache_key)
