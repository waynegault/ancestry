    ...
    try:
        with open(log_filepath, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if "WARNING - [utils check_for_javascript_errors" in line and "429 ()" in line:
                    error_warning_counts["429 Errors"] += 1
                elif "WARNING - [utils nav_to_page line" in line and "Timeout with no specific unavailability reason" in line:
                    error_warning_counts["Navigation Timeouts"] += 1
                elif "ERROR - [action6_gather scrape3 line" in line and "Timeout while extracting relationship path" in line:
                    error_warning_counts["scrape3 Timeouts"] += 1
                elif "WARNING - [action6_gather _extract_tree_name line" in line and "Tree name element not found." in line:
                    error_warning_counts["_extract_tree_name Failures"] += 1
                elif "WARNING - [action6_gather _process_match_details line" in line and "RETRYING _process_match_details." in line:
                    error_warning_counts["_process_match_details Retries"] += 1
                elif "ERROR - [action6_gather _process_match_details line" in line and "SKIPPING MATCH." in line:
                    error_warning_counts["Action 6 Errors"] += 1

                # --- Dynamic Delay Counts ---
                elif "INFO - [action6_gather coord line" in line and "Decreasing delay to" in line:
                    dynamic_delay_counts["Delay Decreases"] += 1
                elif "INFO - [action6_gather coord line" in line and "Maintaining delay" in line:
                    dynamic_delay_counts["Delay Maintained (throttled)"] += 1

                # --- WebDriverWait Timings ---
                elif "DEBUG - [utils wrapper line" in line and "Wait '" in line and "duration" in line:
                    match = re.search(r"Wait '([^']+) duration' completed in (\d+\.\d+) seconds", line)
                    if match:
                ...
                elif "DEBUG - [utils wrapper line" in line and "_process_match_details duration" in line:
                    match = re.search(r"_process_match_details duration (\d+\.\d+) s", line)
                    if match:
                        duration = float(match.group(1))
                        process_match_durations.append(duration)

                # --- nav_to_page Timings ---
                elif "DEBUG - [utils wrapper line" in line and "nav_to_page" in line and "s." in line:
                ...
                # --- scrape2 Timings ---
                elif "DEBUG - [utils wrapper line" in line and "scrape2 duration" in line:
                ...
                elif "DEBUG - [action6_gather coord line" in line and "Page" in line and "batch" in line:
                    match = re.search(r"Page \d+ batch \d+: New (\d+), Update (\d+), Skip (\d+), Error (\d+)", line)
                    if match:
                        ...
                        error_warning_counts["Action 6 Errors"] += int(match.group(4))  # Count errors from summary line

                # --- NEW:  Sub-total Counts (Page Level) ---
                elif "INFO - [action6_gather coord line" in line and "Page" in line and "sub-total" in line:
                    match = re.search(r"Page \d+ sub-total: New (\d+), Update (\d+), Skip (\d+), Error (\d+)", line)
                    # NO IF NEEDED.  Directly add values to counts.  Values will accumulate.
                    if match: #KEEP THIS IF, AS IT CHECKS THE REGEX MATCHED BEFORE PROCEEDING
                        skipped_updated_new_counts["New"] += int(match.group(1))
                        skipped_updated_new_counts["Updated"] += int(match.group(2))
                        skipped_updated_new_counts["Skipped"] += int(match.group(3))
                        error_warning_counts["Action 6 Errors"] += int(match.group(4)) # Accumulate errors.
...
def format_statistics(name, durations):
    ...
    if len(durations) >= 10:
        report += f"    90th Percentile: {statistics.quantiles(durations, n=10)[8]:.3f}\n"
    else:
        report += "    90th Percentile: N/A (Data points < 10)\n"

    if len(durations) >= 100:
        report += f"    99th Percentile: {statistics.quantiles(durations, n=100)[98]:.3f}\n"
    else:
