...
def run_test_action12(_sm: SessionManager):
    separator("Action 12: Triangulation")
    try:
        logger.info("Skipping Action 12 (Interactive) in headless harness.")
        # action12_run(sm)
    except Exception as e:
        logger.error(f"Action 12 error: {e}")


def run_test_action13(sm: SessionManager):
    separator("Action 13: Fetch Shared Matches")
    try:
        original_max_pages = config_schema.api.max_pages
        config_schema.api.max_pages = 1
        logger.info("Running Action 13 with max_pages=1...")
        action13_run(sm)
    except Exception as e:
