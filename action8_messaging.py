...
def send_messages_to_matches(session_manager: SessionManager) -> bool:
    ...
    try:
        with session_manager.get_db_conn_context() as db_session:
            ...
            logger.debug(f"Found {total_potential} potential CONTACTABLE DNA matches to check.")
            ...
            if not potential_person_ids:
                 logger.info("No valid person IDs found among potential matches. Finishing.")
                 return True
            ...
            try:
                ...
                for row in latest_history_results:
                     ...
                     if sent_at_db:
                         sent_at_naive = sent_at_db.astimezone(timezone.utc).replace(tzinfo=None) if sent_at_db.tzinfo else sent_at_db.replace(tzinfo=None)
                         latest_history_data[person_id] = (type_name, sent_at_naive, status)
            ...
            with logging_redirect_tqdm():
                ...
                for match_data in potential_matches:
                    item_skipped = False; item_error = False
                    ...
                    try:
                        ...
                        error_count += 1; overall_success = False; item_error = True
                        if progress_bar:
                            progress_bar.set_postfix(Sent=sent_count, Skip=skipped_count, Err=error_count, refresh=False)
                    ...
                    try: # Inner try for this item's processing
                        ...
                        if inbox_status:
                            ...
                            if received_at_db:
                                last_received_at_local = received_at_db.astimezone(timezone.utc).replace(tzinfo=None) if received_at_db.tzinfo else received_at_db.replace(tzinfo=None)
                        ...
                        if they_sent_last and last_received_at_local:
                             if last_sent_at_local:
                                  if last_received_at_local > last_sent_at_local: logger.debug(f"Skipping {log_prefix}: They replied."); skipped_count += 1; item_skipped = True; raise StopIteration
                             else: logger.debug(f"Skipping {log_prefix}: They sent, no prior script message."); skipped_count += 1; item_skipped = True; raise StopIteration
                        if last_sent_at_local and (now_local - last_sent_at_local) < MIN_MESSAGE_INTERVAL: logger.debug(f"Skipping {log_prefix}: Within interval."); skipped_count += 1; item_skipped = True; raise StopIteration
                        ...
                        if not next_message_type_key: logger.debug(f"Skipping {log_prefix}: No next message type."); skipped_count += 1; item_skipped = True; raise StopIteration
                        message_template = MESSAGE_TEMPLATES.get(next_message_type_key)
                        if not message_template: logger.error(f"Missing template key '{next_message_type_key}'."); error_count += 1; overall_success = False; item_error = True; raise StopIteration
                        ...
                        formatted_name = format_name(name_to_use); logger.debug(f"Using name '{formatted_name}' (Source: {source}) for {log_prefix}")
                        ...
                        for key in req_keys:
                             ...
                             if key in critical_keys and (value_in_final_dict == "N/A" or value_in_final_dict == "Valued Relative"):
                                  if not original_value or (key == "name" and name_to_use == "Valued Relative"): critical_keys_failed.append(f"{key} (original data missing/invalid)")
                        if critical_keys_failed: logger.error(f"CRITICAL formatting keys failed for {log_prefix}: {critical_keys_failed}."); error_count += 1; overall_success = False; item_error = True; raise StopIteration
                        elif keys_using_fallback: logger.warning(f"Formatting {log_prefix}: Fallback used for keys: {keys_using_fallback}.")
                        try: message_text = message_template.format(**format_data)
                        except KeyError as e: logger.error(f"KeyError fmt msg for {log_prefix}: {e}."); error_count += 1; overall_success = False; item_error = True; raise StopIteration
                        except Exception as e: logger.error(f"Error fmt msg for {log_prefix}: {e}."); error_count += 1; overall_success = False; item_error = True; raise StopIteration
                        logger.debug(f"Formatted message for {log_prefix}: '{message_text[:100]}...'")

                        # API Call Prep
                        message_status: str = "skipped (logic_error)"; new_conversation_id_from_api_local: Optional[str] = None
                        is_initial_message: bool = not conversation_id; send_api_url: str = ""; payload: Dict[str, Any] = {}; send_api_desc: str = ""; api_headers = {}
                        ...
                        else: logger.error(f"Logic Error: Cannot determine API URL/payload for {log_prefix}."); error_count += 1; overall_success = False; item_error = True; raise StopIteration
                        ...
                        if config_instance.APP_MODE == "dry_run":
                            logger.debug(f"Dry Run: Would send '{next_message_type_key}' to {log_prefix}")
                            ...
                            action = "create" if is_initial_message else "send follow-up"; logger.info(f"Sending ({action}) '{next_message_type_key}' to {log_prefix}...")
                            api_response: Optional[Any] = None
                            try: api_response = _api_req(url=send_api_url, driver=session_manager.driver, session_manager=session_manager, method="POST", json_data=payload, use_csrf_token=False, headers=api_headers, api_description=send_api_desc, force_requests=True)
                            except Exception as api_e: logger.error(f"Exception during _api_req for {log_prefix}: {api_e}."); message_status = "send_error (api_exception)"; error_count += 1; overall_success = False; item_error = True; raise StopIteration
                            # API Response Validation
                            post_ok = False; api_conv_id: Optional[str] = None; api_author: Optional[str] = None
                            if api_response is not None:
                                if is_initial_message:
                                    if isinstance(api_response, dict) and "conversation_id" in api_response:
                                        ...
                                        if api_conv_id and api_author == MY_PROFILE_ID_UPPER: post_ok = True; new_conversation_id_from_api_local = api_conv_id
                                        else: logger.error(f"API initial response invalid for {log_prefix}.")
                                    else: logger.error(f"API call ({send_api_desc}) unexpected format (initial) for {log_prefix}."); logger.debug(f"Resp:{api_response}")
                                else:
                                    if isinstance(api_response, dict) and "author" in api_response:
                                        ...
                                        if api_author == MY_PROFILE_ID_UPPER:
                                            post_ok = True; new_conversation_id_from_api_local = conversation_id
                                            if api_response.get("content") != message_text: logger.warning(f"API response content mismatch for {log_prefix}.")
                                        else: logger.error(f"API follow-up author validation failed for {log_prefix}.")
                                    else: logger.error(f"API call ({send_api_desc}) unexpected format (follow-up) for {log_prefix}."); logger.debug(f"Resp:{api_response}")
                                if post_ok: message_status = "delivered OK"; logger.debug(f"Message send to {log_prefix} ACCEPTED.")
                                else: message_status = "send_error (validation_failed)"; error_count += 1; overall_success = False; item_error = True; logger.warning(f"API POST validation failed for {log_prefix}."); raise StopIteration
                            else: logger.error(f"API POST ({send_api_desc}) for {log_prefix} failed (No response)."); message_status = "send_error (post_failed)"; error_count += 1; overall_success = False; item_error = True; raise StopIteration
                        else: logger.warning(f"Skipping send for {log_prefix} due to APP_MODE: {config_instance.APP_MODE}"); skipped_count += 1; item_skipped = True; raise StopIteration
                        ...
                        if message_status in ("delivered OK", "typed (dry_run)"):
                            ...
                            try: # DB Prep Try
                                ...
                                if not msg_type_obj: raise ValueError(f"MessageType '{next_message_type_key}' not found.")
                                current_time_for_db = now_local; trunc_hist = message_text[:config_instance.MESSAGE_TRUNCATION_LENGTH]
                                new_history = MessageHistory(people_id=person_id, message_type_id=msg_type_obj.id, message_text=trunc_hist, status=message_status, sent_at=current_time_for_db)
                                ...
                                if not effective_conv_id: logger.error(f"DB Prep Error for {log_prefix}: Conv ID missing!"); error_count += 1; overall_success = False; item_error = True; raise StopIteration
                                ...
                                if inbox_status_upd:
                                    ...
                                    db_ts_naive_inbox = inbox_status_upd.last_message_timestamp.astimezone(timezone.utc).replace(tzinfo=None) if inbox_status_upd.last_message_timestamp and inbox_status_upd.last_message_timestamp.tzinfo else (inbox_status_upd.last_message_timestamp.replace(tzinfo=None) if inbox_status_upd.last_message_timestamp else None)
                                    if db_ts_naive_inbox != current_time_for_db: inbox_status_upd.last_message_timestamp = current_time_for_db; updated_inbox = True
                                    ...
                                    if updated_inbox: inbox_status_upd.last_updated = datetime.now(); logger.debug(f"Staged InboxStatus update for {log_prefix}")
                                else:
                                    new_inbox = InboxStatus(people_id=person_id, conversation_id=effective_conv_id, my_role=RoleType.AUTHOR, last_message=trunc_inbox, last_message_timestamp=current_time_for_db)
                                    db_objects_to_commit.append(new_inbox); inbox_data[person_id] = new_inbox; logger.debug(f"Staged new InboxStatus for {log_prefix}")
                                # Commit periodically
                                if len(db_objects_to_commit) >= db_commit_batch_size:
                                    logger.debug(f"Reached batch size ({db_commit_batch_size}). Committing...")
                                    try: db_session.add_all(db_objects_to_commit); db_session.flush(); logger.debug("Flushed batch."); db_objects_to_commit = []
                                    except Exception as commit_err: logger.error(f"Error flushing batch: {commit_err}"); error_count += len(db_objects_to_commit); db_objects_to_commit = []; overall_success = False; item_error = True; raise StopIteration
                            except ValueError as db_prep_val_err: logger.error(f"DB prep ValueError for {log_prefix}: {db_prep_val_err}."); error_count += 1; overall_success = False; item_error = True; raise StopIteration
                            except SQLAlchemyError as db_prep_sql_err: logger.error(f"DB prep SQLAlchemyError for {log_prefix}: {db_prep_sql_err}."); error_count += 1; overall_success = False; item_error = True; raise StopIteration
                            except Exception as db_prep_unexp: logger.critical(f"Unexpected DB prep error for {log_prefix}: {db_prep_unexp}."); error_count += 1; overall_success = False; item_error = True; raise StopIteration

                    except StopIteration: pass # Graceful exit for skips/errors
            ...
            if db_objects_to_commit:
                ...
                try:
                    db_session.add_all(db_objects_to_commit); db_session.flush()
                    ...
                    error_count += len(db_objects_to_commit); overall_success = False
        ...
        if logger: logger.critical(f"CRITICAL: Critical error during message processing loop: {outer_e}", exc_info=True)
