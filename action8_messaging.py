# REVISED action8_messaging.py - FIXED SyntaxError on Line 397
...
MIN_MESSAGE_INTERVAL = MESSAGE_INTERVALS.get(config_instance.APP_MODE, timedelta(weeks=8)) # Use production as default fallback
...
def determine_next_message_type(
    ...
    """
    # (Function unchanged - Indentation checked)
    logger.debug(f"Determining next msg type (interval/reply checks done by caller):")
    ...
    if is_in_family_tree:
        if last_message_type.startswith("Out_Tree"):
            ...
            logger.debug(f"Context: {skip_reason}.")
        else:
            skip_reason = f"Unexpected previous In_Tree message type: {last_message_type}"
            logger.warning(f"Context: {skip_reason}.")
        ...
        if last_message_type.startswith("In_Tree"):
            ...
            logger.warning(f"Context: {skip_reason}. Skipping.")
            ...
            skip_reason = f"Unexpected previous Out_Tree message type: {last_message_type}"
            logger.warning(f"Context: {skip_reason}.")

    if next_type:
        logger.debug(f"Result: {next_type} (Reason: Following sequence).")
    else:
        logger.info(f"Result: None (Reason: {skip_reason}).")
...
def send_messages_to_matches(session_manager: SessionManager) -> bool:
    """
    V13 REVISED: Sends messages to DNA matches based on criteria, using API calls.
    Stores naive LOCAL time. Fixed previous SyntaxError.
    """
    logger.info("Starting message sending process (API Based - Dual Endpoint)...")
    logger.warning("Using LOCAL time for database storage. Beware of DST changes.") # Add warning
    ...
    processed_count = 0; sent_count = 0; skipped_count = 0; error_count = 0
    potential_matches = []
    max_to_send = config_instance.MAX_INBOX

    try:
        with session_manager.get_db_conn_context() as db_session:
            if not db_session: logger.error("Failed to get DB connection."); return False
            ...
            potential_matches_query = (
                ...
                ).join(Person, DnaMatch.people_id == Person.id).outerjoin(FamilyTree, Person.id == FamilyTree.people_id)
                .filter(Person.profile_id.isnot(None), Person.profile_id != "", Person.profile_id != "UNKNOWN")
                .filter(Person.status == "active").filter(Person.contactable == True).order_by(Person.id)
            )
            ...
            for match_data in potential_matches:
                ...
                is_in_family_tree: bool = False; first_name: Optional[str] = None; person_name_in_tree: Optional[str] = None
                ...
                they_sent_last: bool = False; inbox_role: Optional[RoleType] = None
                new_history_record = None

                if max_to_send != 0 and sent_count >= max_to_send: logger.info(f"Reached MAX_INBOX limit ({max_to_send}). Stopping."); break
                processed_count += 1

                try: # Unpack and store original values
                ...
                # --- Process this match ---
                try:
                    now_local = datetime.now() # Get naive local time

                    # --- DB Queries & Timezone Handling (Using Naive Local) ---
                    try: # Message History Fetch
                        last_sent_query = db_session.query(MessageHistory.sent_at, MessageType.type_name, MessageHistory.status).join(MessageType).filter(MessageHistory.people_id == person_id).order_by(desc(MessageHistory.sent_at)).first()
                        if last_sent_query:
                            sent_at_db = last_sent_query.sent_at
                            if sent_at_db:
                                if sent_at_db.tzinfo is not None: last_sent_at_local = sent_at_db.replace(tzinfo=None); logger.warning(f"Made MessageHistory ts naive for {log_prefix}")
                                else: last_sent_at_local = sent_at_db
                                if last_sent_at_local > now_local + timedelta(minutes=1): last_sent_at_local = now_local; logger.warning(f"MessageHistory ts capped for {log_prefix}")
                                last_message_sent_details = (last_sent_query.type_name, last_sent_at_local, last_sent_query.status)
                                logger.debug(f"Last sent (local): Type={last_sent_query.type_name}, At={last_sent_at_local}, Status={last_sent_query.status}")
                            else: logger.warning(f"MessageHistory found for {log_prefix}, but sent_at is NULL.")
                        else: logger.debug("No MessageHistory found.")
                    except SQLAlchemyError as db_err: logger.error(f"DB error fetching MessageHistory for {log_prefix}: {db_err}", exc_info=True); error_count += 1; overall_success = False; continue
                    except Exception as e: logger.error(f"Error processing MessageHistory for {log_prefix}: {e}", exc_info=True); error_count += 1; overall_success = False; continue

                    try: # Inbox Status Fetch
                        inbox_status = db_session.query(InboxStatus).filter(InboxStatus.people_id == person_id).first()
                        if inbox_status:
                            logger.debug(f"InboxStatus: Role={inbox_status.my_role}, LastMsgAt={inbox_status.last_message_timestamp}, ConvID={inbox_status.conversation_id}")
                            ...
                            if received_at_db:
                                if received_at_db.tzinfo is not None: last_received_at_local = received_at_db.replace(tzinfo=None); logger.warning(f"Made InboxStatus ts naive for {log_prefix}")
                                else: last_received_at_local = received_at_db
                                if last_received_at_local > now_local + timedelta(minutes=1): last_received_at_local = now_local; logger.warning(f"InboxStatus ts capped for {log_prefix}")
                                if inbox_status.my_role == RoleType.RECIPIENT: they_sent_last = True; logger.debug(f"They sent last at (local): {last_received_at_local}")
                            elif inbox_status.my_role == RoleType.RECIPIENT: logger.warning(f"DB Consistency Issue: Inbox role RECIPIENT for {log_prefix}, but timestamp is NULL."); they_sent_last = False
                        else: logger.debug("No InboxStatus found.")
                    except SQLAlchemyError as db_err: logger.error(f"DB error fetching InboxStatus for {log_prefix}: {db_err}", exc_info=True); error_count += 1; overall_success = False; continue
                    except Exception as e: logger.error(f"Error processing InboxStatus for {log_prefix}: {e}", exc_info=True); error_count += 1; overall_success = False; continue

                    if not last_message_sent_details and inbox_role == RoleType.AUTHOR: logger.warning(f"DB Consistency Check: MessageHistory empty for {log_prefix}, but InboxStatus.my_role is AUTHOR.")

                    # --- Reply Check (Using Naive Local Times) ---
                    if they_sent_last and last_received_at_local:
                        if last_sent_at_local:
                            logger.debug(f"Reply Check Comparison (Naive Local) for {log_prefix}:")
                            logger.debug(f"  - last_received_at_local: Value='{last_received_at_local}'")
                            logger.debug(f"  - last_sent_at_local:     Value='{last_sent_at_local}'")
                            if last_received_at_local > last_sent_at_local: logger.info(f"Skipping {log_prefix}: Reply received."); skipped_count += 1; continue
                            else: logger.debug(f"Reply received but before last script msg.")
                        else: logger.info(f"Skipping {log_prefix}: Reply received, but no prior script msg."); skipped_count += 1; continue

                    # --- Interval Check (Using Naive Local Times) ---
                    if last_sent_at_local:
                        time_since = now_local - last_sent_at_local
                        if time_since < MIN_MESSAGE_INTERVAL: logger.info(f"Skipping {log_prefix}: Interval not met."); skipped_count += 1; continue
                        else: logger.debug(f"Interval check passed for {log_prefix}.")
                    ...
                    formatted_name = format_name(name_to_use); logger.debug(f"Using name '{formatted_name}' (Source:{source})")
                    ...
                    logger.debug(f"Formatted msg for {log_prefix} ({next_message_type_key}): '{message_text[:100]}...'")

                    # --- API Call Prep ---
                    message_status: str = "skipped (logic_error)"; new_conversation_id_from_api_local: Optional[str] = None; is_initial_message: bool = not conversation_id
                    logger.debug(f"API action: conv_id='{conversation_id}', initial={is_initial_message}"); send_api_url: str = ""; payload: Dict[str, Any] = {}; send_api_desc: str = ""; api_headers = {}
                    if is_initial_message: send_api_url = urljoin(config_instance.BASE_URL.rstrip('/')+'/', "app-api/express/v2/conversations/message"); send_api_desc = "Create Conversation API"; payload = {"content": message_text, "author": MY_PROFILE_ID_LOWER, "index": 0, "created": 0, "conversation_members": [{"user_id": recipient_profile_id_upper.lower(), "family_circles": [] }, {"user_id": MY_PROFILE_ID_LOWER}]}
                    elif conversation_id: send_api_url = urljoin(config_instance.BASE_URL.rstrip('/')+'/', f"app-api/express/v2/conversations/{conversation_id}"); send_api_desc = "Send Message API (Existing Conv)"; payload = {"content": message_text, "author": MY_PROFILE_ID_LOWER}
                    ...
                    # --- Execute based on APP_MODE ---
                    if config_instance.APP_MODE == "dry_run":
                        action = "create" if is_initial_message else "send to"
                        logger.info(f"[DRY RUN] Would {action} '{next_message_type_key}' to {log_prefix}.")
                    ...
                    # --- V13 FIX: Corrected SyntaxError ---
                    elif config_instance.APP_MODE in ["production", "testing"]:
                        action = "create" if is_initial_message else "send follow-up"
                        logger.info(f"Attempting to {action} '{next_message_type_key}' to {log_prefix} via API...")
                        ...
                        if api_response is not None:
                            if is_initial_message:
                                if isinstance(api_response, dict) and 'conversation_id' in api_response:
                                    api_conv_id = str(api_response.get('conversation_id')); msg_details = api_response.get('message', {});
                                    if isinstance(msg_details, dict):
                                        api_author = str(msg_details.get('author', '')).upper()
                                    if api_conv_id and api_author == MY_PROFILE_ID_UPPER:
                                        post_ok = True; author_match = True; new_conversation_id_from_api_local = api_conv_id; logger.info(f"API POST OK (initial): ConvID {new_conversation_id_from_api_local}.")
                                    else:
                                        logger.error(f"API initial response invalid. ConvID:'{api_conv_id}', Author:'{api_author}'")
                                else:
                                    # --- FIX: Corrected Indentation for this block ---
                                    logger.error(f"API call ({send_api_desc}) unexpected format (initial). Type:{type(api_response)}")
                                ...
                                if isinstance(api_response, dict) and 'author' in api_response:
                                    ...
                                    if api_author == MY_PROFILE_ID_UPPER:
                                        post_ok = True; author_match = True; new_conversation_id_from_api_local = conversation_id; logger.info(f"API POST OK (follow-up).")
                                        resp_content = api_response.get('content');
                                        if resp_content != message_text: logger.warning(f"API response content mismatch.")
                                    else:
                                        logger.error(f"API follow-up author validation failed. Author:'{api_author}'")
                                else:
                                    # --- FIX: Corrected Indentation for this block ---
                                    logger.error(f"API call ({send_api_desc}) unexpected format (follow-up). Type:{type(api_response)}")
                                    logger.debug(f"Resp:{api_response}")
                                    # --- End Fix ---
                            if post_ok and author_match:
                                message_status = "delivered OK"; logger.info(f"Message send to {log_prefix} ACCEPTED.")
                            else:
                                message_status = "send_error (validation_failed)"; error_count += 1; overall_success = False; logger.warning(f"API POST validation failed for {log_prefix}.")
                        else:
                             logger.error(f"API POST ({send_api_desc}) for {log_prefix} failed."); message_status = "send_error (post_failed)"; error_count += 1; overall_success = False
                    # --- End V13 FIX ---
                    else:
                    ...
                    if message_status == "delivered OK" or message_status == "typed (dry_run)":
                        logger.debug(f"Effective Conv ID for DB update: {new_conversation_id_from_api_local}")
                        new_history_record = None
                        try:
                            ...
                            current_time_local = now_local # Use naive local time
                            logger.debug(f"Using naive local timestamp for DB: {current_time_local}")
                            trunc_hist = message_text[:config_instance.MESSAGE_TRUNCATION_LENGTH]
                            new_history_record = MessageHistory(people_id=person_id, message_type_id=msg_type_obj.id, message_text=trunc_hist, status=message_status, sent_at=current_time_local)
                            db_session.add(new_history_record); logger.debug(f"Added MessageHistory for {log_prefix}.")
                            if message_status in ["delivered OK", "typed (dry_run)"]:
                                inbox_status_upd = db_session.query(InboxStatus).filter(InboxStatus.people_id == person_id).first()
                                effective_conv_id = new_conversation_id_from_api_local
                                if not effective_conv_id: logger.error(f"DB Update Error for {log_prefix}: Conv ID missing!"); error_count += 1; overall_success = False; continue
                                trunc_inbox = message_text[:config_instance.MESSAGE_TRUNCATION_LENGTH]
                                if inbox_status_upd:
                                    updated = False
                                    if inbox_status_upd.my_role != RoleType.AUTHOR: inbox_status_upd.my_role = RoleType.AUTHOR; updated = True
                                    if inbox_status_upd.last_message != trunc_inbox: inbox_status_upd.last_message = trunc_inbox; updated = True
                                    db_ts_naive = None;
                                    if inbox_status_upd.last_message_timestamp:
                                        if inbox_status_upd.last_message_timestamp.tzinfo is not None: db_ts_naive = inbox_status_upd.last_message_timestamp.replace(tzinfo=None)
                                        else: db_ts_naive = inbox_status_upd.last_message_timestamp
                                    if db_ts_naive != current_time_local: inbox_status_upd.last_message_timestamp = current_time_local; updated = True
                                    if inbox_status_upd.conversation_id != effective_conv_id: inbox_status_upd.conversation_id = effective_conv_id; updated = True; logger.debug(f"Updated Inbox Conv ID for {log_prefix}")
                                    if updated: inbox_status_upd.last_updated = now_local; logger.debug(f"Updated InboxStatus for {log_prefix}.")
                                    ...
                                    db_session.add(new_inbox); logger.debug(f"Created InboxStatus for {log_prefix}.")
                            # Flush changes to assign ID
                            db_session.flush()
                            logger.info(f"DB changes staged for {log_prefix} (Status:{message_status}).")
                            if new_history_record and new_history_record.id: logger.debug(f"Successfully staged MessageHistory with ID: {new_history_record.id}")
                            elif new_history_record: logger.warning(f"MessageHistory staged, but ID not assigned after flush for {log_prefix}.")
                            if message_status == "delivered OK": sent_count += 1
                        except ValueError as val_err: logger.error(f"DB update validation err for {log_prefix}: {val_err}", exc_info=True); error_count += 1; overall_success = False; continue
                        except SQLAlchemyError as db_e: logger.error(f"DB err staging updates for {log_prefix}: {db_e}", exc_info=True); error_count += 1; overall_success = False; continue
                        except Exception as db_e_unexp: logger.critical(f"Unexpected DB err staging updates for {log_prefix}: {db_e_unexp}", exc_info=True); error_count += 1; overall_success = False; continue

                    # --- Delay ---
                    if message_status.startswith(("delivered", "typed", "send_error")):
                        delay_td = MESSAGE_INTERVALS.get(config_instance.APP_MODE, timedelta(seconds=5)); delay = delay_td.total_seconds()
                        if config_instance.APP_MODE != "dry_run": delay = random.uniform(delay * 0.8, delay * 1.2)
                        logger.debug(f"Waiting {delay:.2f}s..."); time.sleep(delay)
                    else: logger.debug("Skipping delay.")

                except Exception as inner_loop_e: # Catch errors within single match processing
                    if isinstance(inner_loop_e, TypeError) and "can't compare offset-naive and offset-aware datetimes" in str(inner_loop_e): logger.critical(f"CRITICAL DATETIME COMPARISON ERROR for {log_prefix}: {inner_loop_e}", exc_info=True); logger.error("Aborting this match.")
                    elif isinstance(inner_loop_e, TypeError) and ("replace() got an unexpected keyword argument 'tzinfo'" in str(inner_loop_e) or "astimezone() cannot be applied to a naive datetime" in str(inner_loop_e)): logger.critical(f"CRITICAL DATETIME CONVERSION ERROR for {log_prefix}: {inner_loop_e}", exc_info=True); logger.error("Aborting this match.")
                    else: logger.error(f"Unexpected error processing {log_prefix}: {inner_loop_e}", exc_info=True)
                    error_count += 1; overall_success = False; continue

            logger.info("Finished processing all potential matches.")
...
def main():
    ...
    try:
        ...
        if 'logger' not in globals() or not isinstance(logger, logging.Logger) or not logger.hasHandlers():
            ...
            if logger and isinstance(logger, logging.Logger): logger.setLevel(logging.DEBUG)
            else: logger = logging.getLogger("logger"); logger.setLevel(logging.DEBUG)
        logger.info(f"--- Starting Action 8 Standalone Test ---")
        logger.info(f"APP_MODE: {config_instance.APP_MODE}")
    except Exception as log_setup_e:
        import sys; import logging as pylogging
        print(f"CRITICAL: Error during logging setup: {log_setup_e}", file=sys.stderr)
        pylogging.basicConfig(level=pylogging.DEBUG); logger = pylogging.getLogger("Action8Fallback")
    ...
    try:
        ...
        if start_ok:
            ...
            if action_success: logger.info("send_messages_to_matches completed successfully.")
            else: logger.error("send_messages_to_matches reported errors/failed.")
        else:
        ...
        logger.info(f"--- Action 8 Standalone Test Finished (Overall Success: {action_success}) ---")
# end main
