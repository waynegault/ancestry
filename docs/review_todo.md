...
`main.py` is still ~3,500 lines and mixes menu rendering, caching bootstrap, analytics launchers, and CLI helpers (`main.py` §1‑3300). On Nov 25 we finally moved the `exec_actn` orchestration (state detection, analytics logging, metrics) into `core/action_runner.py` so `main.py` imports shared helpers instead of maintaining a private copy. As of Nov 21 the non-essential CLI utilities live in `cli/maintenance.py`, and the `main_module_tests()` suite is back in `main.py` to satisfy the co-location rule, but the entrypoint still carries caching bootstrap logic plus various analytics launchers.
