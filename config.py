...
class Config_Class(BaseConfig):
    ...
    MAX_DELAY: float = 60.0      # Lower max delay slightly
    ...
    def _load_values(self):
        ...
        self.GEDCOM_FILE_PATH: Optional[Path] = self._get_path_env("GEDCOM_FILE_PATH", None)
        self.CACHE_DIR_PATH: Path = self.CACHE_DIR
        logger.debug(f"Database file path: {self.DATABASE_FILE.resolve()}")
        if self.GEDCOM_FILE_PATH: logger.debug(f"Gedcom file path: {self.GEDCOM_FILE_PATH.resolve()}")
        else: logger.debug("Gedcom file path: Not set")

        # === URLs ===
        self.BASE_URL: str = self._get_string_env("BASE_URL", "https://www.ancestry.co.uk/")
        self.ALTERNATIVE_API_URL: Optional[str] = self._get_env_var("ALTERNATIVE_API_URL")
        self.API_BASE_URL_PATH: str = self._get_string_env("API_BASE_URL_PATH", "/api/v2/") # Example default
        ...
        if self.BASE_URL and self.API_BASE_URL_PATH: # Check dependencies are loaded
             ...
             logger.debug("API_BASE_URL_PATH not set, API_BASE_URL will not be constructed.")
             self.API_BASE_URL = None
        else:
             logger.error("BASE_URL not loaded correctly, cannot construct API_BASE_URL.")
             self.API_BASE_URL = None
        # --- END MOVE ---
        ...
        # === Database ===
        self.DB_POOL_SIZE: int = self._get_int_env("DB_POOL_SIZE", self.DB_POOL_SIZE) # Use class default

        # === Caching ===
        self.CACHE_TIMEOUT: int = self._get_int_env("CACHE_TIMEOUT", 3600) # 1 hour default
        ...
        self.DECREASE_FACTOR = self._get_float_env("DECREASE_FACTOR", self.DECREASE_FACTOR)
        logger.debug(
        ...
        if isinstance(retry_codes_from_env, (list, tuple)):
             if all(isinstance(code, int) for code in retry_codes_from_env):
                 ...
                 logger.warning(f"Invalid non-integer value in RETRY_STATUS_CODES env var. Using default: {self.RETRY_STATUS_CODES}")
        else:
            logger.warning(f"RETRY_STATUS_CODES env var is not valid JSON list/tuple. Using default: {self.RETRY_STATUS_CODES}")
        ...
        logger.info(f"Configuration Loaded: BASE_URL='{self.BASE_URL}', DB='{self.DATABASE_FILE.name}', TREE_NAME='{self.TREE_NAME or 'Not Set'}'")
        ...
        logger.debug(f"API Base Path: '{self.API_BASE_URL_PATH or 'Not Set'}' -> API Base URL: '{self.API_BASE_URL or 'Not Constructed'}'")
...
class SeleniumConfig(BaseConfig):
    ...
    def _load_values(self):
        ...
        self.LOGGED_IN_CHECK_TIMEOUT = self._get_int_env("LOGGED_IN_CHECK_TIMEOUT", self.LOGGED_IN_CHECK_TIMEOUT)
        self.MODAL_TIMEOUT = self._get_int_env("MODAL_TIMEOUT", self.MODAL_TIMEOUT)
        self.DNA_LIST_PAGE_TIMEOUT = self._get_int_env("DNA_LIST_PAGE_TIMEOUT", self.DNA_LIST_PAGE_TIMEOUT)
        self.NEW_TAB_TIMEOUT = self._get_int_env("NEW_TAB_TIMEOUT", self.NEW_TAB_TIMEOUT)
        self.TWO_FA_CODE_ENTRY_TIMEOUT = self._get_int_env("TWO_FA_CODE_ENTRY_TIMEOUT", self.TWO_FA_CODE_ENTRY_TIMEOUT)
