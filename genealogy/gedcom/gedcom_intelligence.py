...
class GedcomIntelligenceAnalyzer:
    ...
    def analyze_gedcom_data(self, gedcom_data: Any) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"GEDCOM analysis completed: {len(self.gaps_identified)} gaps, {len(self.conflicts_identified)} conflicts, {len(self.opportunities_identified)} opportunities"
            )
    ...
    def _analyze_relationship_conflicts(self, gedcom_data: Any):
        ...
        for person_id, parents in parent_map.items():
            try:
                ...
                person_birth_year = self._extract_birth_year(person_record) if person_record else None
                if not person_birth_year:
                    continue

                for parent_id in parents:
                    parent_record = gedcom_data.indi_index.get(parent_id)
                    parent_birth_year = self._extract_birth_year(parent_record) if parent_record else None
...
def test_gap_detection_with_mocked_birth_year() -> None:
    ...
    assert 'missing_parents' in gap_types or 'missing_parents' in ''.join(gap_types), (
        "Should include missing parents gap"
    )
    assert any('Missing birth location' in g['description'] for g in result['gaps_identified']), (
        "Should include missing place gap"
    )
...
def test_invalid_gedcom_data() -> None:
    ...
    empty_gedcom = type('EmptyGedcom', (), {'indi_index': {}, 'id_to_parents': {}, 'id_to_children': {}})()
...
def test_edge_case_dates() -> None:
    ...
    mock_person = type(
        'Person', (), {'name': ['Test Person'], 'birth': [{'date': 'invalid date'}], 'death': [{'date': '1900'}]}
    )()

    mock_gedcom = type(
        'MockGedcom', (), {'indi_index': {'I1': mock_person}, 'id_to_parents': {}, 'id_to_children': {}}
    )()
...
def test_special_characters_in_names() -> None:
    ...
    mock_gedcom = type('MockGedcom', (), {'indi_index': mock_people, 'id_to_parents': {}, 'id_to_children': {}})()
...
def test_large_dataset_performance() -> None:
    ...
    mock_gedcom = type('LargeGedcom', (), {'indi_index': large_indi_index, 'id_to_parents': {}, 'id_to_children': {}})()
...
run_comprehensive_tests = create_standard_test_runner(gedcom_intelligence_module_tests)
