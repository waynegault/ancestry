...
def _test_module_integrity() -> bool:
    ...
    suite = TestSuite("genealogy __init__", "genealogy/__init__.py")

    def _make_import_test(sub: str):
        def test():
            mod = importlib.import_module(f"genealogy.{sub}")
            assert mod is not None, f"genealogy.{sub} should import successfully"
            assert hasattr(mod, "__name__"), f"genealogy.{sub} should have __name__"
        return test

    for submodule in sorted(_SUBMODULES):
        suite.run_test(f"{submodule} subpackage imports successfully", _make_import_test(submodule))
