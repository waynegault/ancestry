...
from core.session_utils import ensure_session_for_tests_sm_only
...
def fetch_tree_owner_ethnicity_regions(
    ...
    """
    url = urljoin(
        config_schema.api.base_url, API_PATH_ETHNICITY_OWNER.format(tree_owner_test_guid=tree_owner_test_guid)
    )
...
def fetch_ethnicity_region_names(
    ...
    """
    url = urljoin(config_schema.api.base_url, API_PATH_ETHNICITY_REGION_NAMES.format(locale=locale))
...
def fetch_ethnicity_comparison(
    session_manager: SessionManager, tree_owner_test_guid: str, match_test_guid: str
) -> dict[str, Any] | None:
    """
    ...
    url = urljoin(
        ...
        API_PATH_ETHNICITY_COMPARE.format(tree_owner_test_guid=tree_owner_test_guid, match_test_guid=match_test_guid),
    )
    ...
    try:
        ...
        for region in regions:
            ...
            tree_owner_regions.append(
                {"key": region_key, "name": region_name, "percentage": percentage, "column_name": column_name}
            )
...
def _add_ethnicity_columns_to_database(
    tree_owner_regions: list[dict[str, Any]], db_manager: SessionManager | None = None
) -> bool:
    """
...
def extract_match_ethnicity_percentages(
    comparison_data: dict[str, Any], tree_owner_region_keys: list[str]
) -> dict[str, int | None]:
    """
    ...
    comparison_lookup = {
        comp["resourceId"]: comp.get("rightSum", 0) for comp in comparison_data["comparisons"] if "resourceId" in comp
    }
    ...
    for region_key in tree_owner_region_keys:
        ...
        try:
            normalized_value: int | None = int(percentage_value)
...
def _navigate_to_dna_page(sm: SessionManager) -> bool:
    ...
    from genealogy.dna.dna_utils import nav_to_dna_matches_page
...
def _test_regression_endpoint_literals_present_in_source() -> bool:
    ...
    try:
        ...
        assert "from api.api_constants import" in content
