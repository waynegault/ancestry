...
class APIConfig:
    ...
    # Rate limiting (CRITICAL - Zero Tolerance for 429 errors)
    ...
    token_bucket_capacity: float = 20.0  # Token bucket capacity for burst handling (replaces burst_limit)
    rate_limiter_429_backoff: float = 0.80  # Multiplier applied to rate on 429 error (20% reduction - Stronger backoff)
...
class HealthConfig:
    ...
    # API Response Time Thresholds (seconds)
    # Increased defaults to accommodate aggressive rate limiting/throttling
    api_response_time_warning: float = 5.0
    api_response_time_critical: float = 15.0
    ...
    memory_usage_warning: float = 800.0
...
class ConfigSchema:
    ...
    def validate(self) -> list[str]:
        ...
        try:
            ...
            HealthConfig(**asdict(self.health))
            RetryPoliciesConfig(
...
def _test_rate_limiting_configuration() -> None:
    ...
    validation_rules = [
        ...
        ("burst_limit", lambda v: v > 20, "too high"),
