...
class AlignedMessageFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        try:
            ...
            logger_for_setup.warning("Placeholder 'X' not found in formatted prefix calculation, using fallback.", exc_info=False) # Use setup logger here
...
def setup_logging(log_file: str = "app.log", log_level: str = "INFO") -> logging.Logger:
    ...
    if logger_for_setup.hasHandlers():
        for handler in logger_for_setup.handlers[:]: logger_for_setup.removeHandler(handler)
    ...
    try:
        ...
        if urllib3_logger.hasHandlers():
             logger_for_setup.debug(f"Removing {len(urllib3_logger.handlers)} default handlers from urllib3 logger.")
