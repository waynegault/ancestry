...
class AlignedMessageFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # Handle case where prefix exists but message is empty
            ...
            return (
                prefix_string.rstrip()
            )  # Return just the prefix (remove trailing space/newline)
...
def setup_logging(log_file: str = "app.log", log_level: str = "INFO") -> logging.Logger:
    ...
    try:
        ...
        logging.getLogger("urllib3.connectionpool").setLevel(
            logging.WARNING
        )  # Target the specific pool logger
        logging.getLogger("selenium.webdriver.remote.remote_connection").setLevel(
            logging.INFO
        )
        logging.getLogger("selenium").setLevel(logging.INFO)
        logging.getLogger("websockets").setLevel(logging.INFO)
        logging.getLogger("undetected_chromedriver").setLevel(
            logging.WARNING
        )  # Added undetected_chromedriver
        ...
        logger_for_setup.debug(
            "Set external library log levels and disabled propagation."
        )
